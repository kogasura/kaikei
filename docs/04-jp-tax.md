...
実物は `crates/kaikei-jp-data/tax/jp/2026-01-01.yaml` と `2026-10-01.yaml`。

**ファイル名は暦年ではなく施行日**である（`DECISIONS.md` D-092）。消費税の
改正は暦年の途中に施行されるため、1つの暦年に2つのマスタが並ぶ年がある
（2026年は経過措置の控除割合が 10/1 に 80% → 70% へ変わる）。

改正が来たら、**施行日を名前にしたファイルを追加し、直前のファイルに
`applies_to` を入れる**。直前を閉じないと期間が重なり、`TaxRuleSets::new` が
構築を拒否する（D-054）。逆に1日ずらすと**どのマスタにも属さない日**ができ、
その日の記帳で初めて `NoApplicableRuleSet` が出る。どちらも
`crates/kaikei-jp/tests/tax_master_timeline.rs` が検査する。

### スキーマ

```yaml
version: 1
applies_from: 2026-10-01      # この日以降の取引に適用（施行日）
applies_to: null              # null なら無期限。次のマスタを足すときに埋める
...
**控除できない部分の帳簿処理は実装しない**（`DECISIONS.md` D-059。維持）。
...
**保留の理由は 2026-08-12 の一次情報調査で変わった**（`docs/09-tax-research.md`）。
当初想定していた「仮払消費税を減らして本体に足す」方式は、**国税庁の
原則的取扱いそのもの**であることが確認できた——税抜経理では経過措置割合
相当額だけを仮払消費税等として計上し、残りは本体価額（費用または資産の
取得価額）に含める（所得税消費税経理通達3の2、令和3年2月9日付 経過的
取扱い(2)、消費税経理通達関係Q&A 問3）。記帳の都度行ってよいことも
明文にある（同通達4「原則として取引の都度行う」）。

それでも実装しないのは、**消費税経理通達関係Q&A が令和8年度税制改正
（控除割合 70%）を反映していない**ためである（令和5年12月改訂のままで、
令和8年10月以後を「50%」と記載している）。取扱いの方法は変わらないと
考えられるが、改正後の平成30年改正令附則11の読替え内容が一次情報で
確認できていない。**反映の形が確認できるまで金額を確定させない。**

なお、国税庁は代替の経理も明示的に認めている（10%全額を仮払消費税等に
計上して決算で調整する／その取引だけ区分経理しない）。**どれを採るかは
事業者の選択**であり、実装する際も既定を黙って押しつけない（D-082 と同じ）。

### 簡易課税（`simplified_taxation`）の扱い

**みなし仕入率を記帳に反映しないのは、保留ではなく正しい設計である。**
消費税法37条1項は「30条から前条までの規定により……控除することができる
……税額の合計額は、これらの規定にかかわらず」と定める**申告計算の規定**で
あり、期中の仕訳を変える規定ではない（`docs/09-tax-research.md` 項目10）。
設定は保持し `derive_tax_lines` の挙動は変えない。

ただし帳簿システムとして**未実装**の部分がある。いずれも Phase 4 以降:

- 簡易課税のとき、仕入側の税額を**消費税の申告集計から除外**すること
  （仕訳自体は所得税のために要る）
- 売上仕訳への**事業区分**（第1〜6種）の付与と、未区分の検知。事業区分は
  「課税資産の譲渡等ごと」に判定し（消費税法基本通達13-2-1）、未区分の
  部分には**最も低いみなし仕入率が強制適用される**
- 2割特例（個人事業者は令和8年分が最後）／3割特例（令和9・10年分。
  令和8年度改正で新設）を、恒久フラグではなく**課税期間ごとの選択**として
  持つこと。どちらも事前届出が不要で、申告書への付記のみで適用できる
