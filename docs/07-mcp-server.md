...
| `propose_depreciation_entries` | **Phase 5** | 減価償却費の仕訳の**提案**。固定資産台帳（`fixed_assets`）から年度の償却費を計算し、`借方 減価償却費 / 貸方 それぞれの資産の科目` の1本にまとめる。**記帳はしない**（`posted` は常に `false`。`propose_closing_entries` と同じ形）。**耐用年数も償却方法も台帳に入っている値をそのまま使う**——このツールは推定しない（D-103）。台帳が空なら提案も空だが、「台帳が空」と「その年度に償却するものが無い」は次にやることが違うので、`asset_count` を添えて区別できるようにする。除却した年**以降**の資産は対象外（D-108）|
| `list_pending_transactions` | **Phase 4** | 取り込んだ明細のうち未仕訳のもの。**延期理由は解けた**——`kaikei-import` と `imported_transactions`（0011）を実装した。**返すのは明細であって仕訳ではない**（`docs/05-csv-import.md` §1）。借方・貸方・勘定科目は含まれず、あるのは入金か出金か（`is_money_in`）だけで、どの科目に立てるかはこの後の仕事である。**0件は成功**（空配列）。ただし「全部片付いた」と「まだ1件も取り込んでいない」で次にやることが正反対になるので、状態ごとの件数（`counts`）を添えて区別できるようにする。`status` は `pending` / `journalized` / `ignored` のみを受け、`Pending` のような惜しい指定は空ではなく**エラー**——黙って0件にすると「未処理は無い」と読み違える。`date_from > date_to` も**エラー**（`search_documents` と同じ規律）。金額は文字列で返し、常に正 |
| `journalize_transaction` | **Phase 4** | 取り込んだ明細を仕訳にする。**記帳と状態遷移を同じトランザクションで行う**のがこのツールの存在理由である（`post_journal_entry` と別呼び出しにすると、間で落ちたときに帳簿に仕訳だけが残って明細は未処理のままになり、次の仕訳化が二重計上になる。帳簿は追記のみなので手で逆仕訳を起こすまで金額がずれ続ける）。取引日は明細から採り、`entry_date` で上書きできる（カードの引落日と購入日は違う）。摘要も省略すれば明細のものを使う。**未処理の明細だけ**を扱う——仕訳済みを塗り替えると、先に作った仕訳が帳簿に残ったまま誰からも参照されなくなる。**金額の取り違えを見つける**: 口座が動いた側に明細と同じ金額の行が1つも無ければエラー。合計では見ない（振込手数料が差し引かれた入金のように、合計が一致しない仕訳は正当に存在する）。意図して一致しない場合は `allow_amount_mismatch` で通す |
| `suggest_journal_entry` | **Phase 4** | 取り込んだ明細に、**過去に自分がどう記帳したか**を根拠として仕訳の候補を出す。**記帳しない**（`suggest_tax_category` と同じ「候補と根拠まで、確定は人間に残す」形。D-087）。既存の会計ソフトも学習型の自動仕訳を持つが、**なぜその科目にしたかを説明できない**——ここが返すのは科目だけでなく、どの摘要で探して過去のどの仕訳が当たったかである。摘要の全体で当たらなければ**摘要から取り出した語**で探し直し、どちらで当たったかを `matched_by` に載せる（緩めたことを隠すと、たまたま当たった別の取引を「よく似ている」と読んでしまう）。**赤伝と取り消された仕訳は根拠にしない**（取り消したということは誤りだったということで、繰り返す理由が無い）。**0件は成功**（空配列）。`confidence` は件数と揃い方から決めた目安で、`high` でも確定ではない——過去の記帳が間違っていれば同じ間違いを繰り返す候補になる。**金額は候補に含めない**（按分するかどうかは人が決める） |
...
| `close_period` | Phase 4 以降 | 期間を締める | **延期理由: 前工事が大きい。** `ROADMAP.md` の Phase 3 成果物に `close_period` は無い。実装するには `period_snapshots` の NOT NULL 列（`balances` JSONB / `entry_count` / `last_entry_no` / `checksum`）を埋める必要があり、~~期間内の仕訳を列挙するポートが存在しない~~ **これは解消した**（`JournalRepo::list_entries_in_period`）。残る前工事は `period_snapshots` の NOT NULL 列を埋めることと、canonical JSON の定義である。canonical JSON の正規化・ハッシュ連鎖・新ユースケース・append-only の権限テストが芋づるで要る。`dry_run` 応答の `pending_transactions` は Phase 4（`kaikei-import`）依存。なお **checksum の計算式自体は `docs/03-database.md` §2「checksum の計算方法」に定義済み**（`h_i = sha256(h_{i-1} || canonical_json(entry_i))`）。ただし `canonical_json` が対象とする仕訳の JSON 形は未定義であり、Phase 5 の `kaikei verify` と揃える必要がある（D-070）。`confirm: true` 必須・不可逆という要件は実装時に維持する。仕様案は付録 A |
