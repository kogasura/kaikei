...
| `sort_order` / `active` 列もテンプレートから設定する | テンプレートの `sort` はドメイン変換で破棄済み（D-061）で、`kaikei_core::AccountDef` に対応するフィールドが無い。`active` は無効化のためのユーザー操作用の列であり、投入経路が触ると「無効化した科目が起動のたびに復活する」ことになる |
