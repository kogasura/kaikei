...
## D-088 検索・元帳は取り消された仕訳を隠さず、絞り込み軸は `aggregatable` のキーに限る

**決定**:

1. **`search_entries` / `get_ledger` は、赤伝（逆仕訳）で訂正された仕訳も
   その赤伝も両方返す。** そのうえで、どちらであるかが必ず判別できる欄を付ける。

   | 欄 | 付く相手 | 意味 |
   |---|---|---|
   | `reversed_by`（`entry_id` / `entry_no` / `entry_date`） | 取り消された**元仕訳** | この仕訳は既に赤伝で取り消されている |
   | `reverses`（仕訳IDの UUID 正準表記） | **赤伝** | この仕訳が訂正している対象 |
   | `reverse_reason` | **赤伝** | 訂正理由（記帳時の入力のまま） |
...
2. **`search_entries` の `tags` に指定できるのは `aggregatable: true` の
   タグキーだけ**にする（現状は `tax_category` / `counterparty` / `project`）。
   それ以外・未登録のキーは `not_aggregatable` / `unknown_tag_key` で拒否する。
...
| 候補 | 却下理由 |
|---|---|
| 取り消された仕訳を検索結果・元帳から**除外**する（既定で隠す） | 帳簿が append-only であること自体を presentation 層が打ち消すことになる。除外した結果の合計は帳簿の合計と一致せず、`get_trial_balance` と `get_ledger` が違う数字を返す。何より「訂正の履歴が残る」ことが本プロジェクトの存在意義であり（`CLAUDE.md` §2）、それを読めなくする既定値を置かない |
| 除外するかどうかを引数（`include_reversed`）で選ばせる | 既定値をどちらにしても片方が「黙って隠す」になる。AI は既定値で呼ぶので、既定が「隠す」なら上と同じ問題が起き、既定が「隠さない」なら引数は使われない。**判別できる形で全部返す**方が、呼び出し元が用途に応じて絞れる |
| `reversed_by` を真偽値（`is_reversed: true`）にする | 「どの赤伝で取り消されたか」を追えない。AI が次に取る行動（その赤伝を `get_entry` で開く、二重訂正を避ける）に必要なのは**仕訳ID**であり、真偽値では引けない（`reverse_journal_entry` の `AlreadyReversed` に `reversal_id` を足した PR-B 2巡目と同じ理由） |
| 元仕訳と赤伝を1件に畳んで返す（相殺後の姿を返す） | 相殺は解釈であって記録ではない。畳んだ結果は帳簿のどの行とも対応せず `entry_id` を持てない。提案と確定を分ける（`docs/07-mcp-server.md` §1 ②）のと同じで、**解釈は呼び出し元に残す** |
| 登録済みのタグキーなら何でも絞り込めるようにする | `group_by` に使えるキーと絞り込みに使えるキーが食い違い、AI から見た帳簿の切り口が2種類になる。「`business_ratio` で絞れるのに `group_by` できない」理由を説明する語彙がどこにも無い。`CLAUDE.md` §4 は「集計軸に使うキーは `aggregatable: true` を宣言する」と定めており、**絞り込みは集計軸の切り出し**である |

**理由**（決定1）: 帳簿は追記のみなので、取り消された仕訳は消えずに検索結果と
元帳に出続ける。**それが「取り消し済み」だと読み取れないと、AI は同じ仕訳を
もう一度訂正しようとする。** `reverse_journal_entry` は
`allow_double_reversal: true` を明示すれば二重訂正を通すので、
「取り消されていることに気づかない → 訂正を試みる → 拒否される →
`allow_double_reversal` を付けて再送する」という経路が実在する
（拒否のメッセージ自身がその引数を案内している）。
...
**どちらの赤伝が付いたかも見る**（PR-H レビュー2巡目）。`reversed_by` は
下のトレードオフのとおり**最も古い赤伝1件**であり、それを決めているのは
`LEFT JOIN LATERAL` の中の `ORDER BY r.entry_date, r.entry_no, r.id` である。
この並びを降順にすると**最新の赤伝**が付くが、**行数も残高も1つも変わらない**
ため、「同じ仕訳が2件返らない」ことしか見ていない検査は緑のまま通る（実測）。
初版は `search_entries` 側でしかどちらの赤伝かを見ておらず、`get_ledger` 側は
出現回数と `closing_balance` しか見ていなかった。**両方のツールで、
元帳の行にも `reversed_by` を突き合わせる。**

**決定2（タグの絞り込み）はキー2つ以上で見る**（PR-H レビュー2巡目）。
複数のタグを指定した場合は**すべてを満たす**仕訳だけが返る
（SQL は `NOT EXISTS ( … WHERE NOT EXISTS ( … ))`）。
**キーが1つの検査では AND と OR が一致してしまう**ので、
「いずれかを満たせばよい」への書き換えを捕まえられない（実測）。
差分テストと e2e の土台に、**片方のキーだけを満たす仕訳**と
**両方を満たす仕訳**を置き、2キーで絞り込んだ結果を突き合わせる。
...
- 2件目以降の赤伝は `reversed_by` に現れない（最も古い1件だけ）。二重訂正は
  既定で拒否される例外的な操作であり、1件返れば「取り消されている」という
  判断には足りる。全件が要るようになったら配列に変えればよい（追加は非破壊的）。
- `invoice_reg_no`（登録番号）では仕訳を引けない（`aggregatable: false`）。
  必要になったら `tags.yaml` の宣言を変えるか、絞り込み専用の宣言を新設するかを
  その時点で判断する（どちらも `kaikei-jp` 側の変更で済む）。

---

## D-089 ページングは keyset（カーソル）にし、上限で切ったことを応答から必ず読み取れるようにする

**決定**:

1. **オフセットではなく keyset（カーソル）ページング**にする。
   位置は `search_entries` が `(entry_date, entry_no, entry_id)`、
   `get_ledger` が `(entry_date, entry_no, entry_id, line_no)` の全順序で、
   次のページは「この位置より厳密に後ろ」から始まる。
   線上の表現は `kaikei_app::wire::{entry_cursor_to_string,
   entry_cursor_from_string, ledger_cursor_to_string, ledger_cursor_from_string}`
   に置く（線上語彙は `kaikei-app` に1箇所。D-072）。
2. **件数の上限は `kaikei-app` のユースケースが持つ**
   （`search_entries` は既定 20 / 上限 100 件、`get_ledger` は既定 100 /
   上限 500 行）。上限を超える `limit` は**丸めずに拒否する**。
3. **切ったことは応答から必ず読み取れる。**
...
| 候補 | 却下理由 |
|---|---|
| オフセットページング（`offset` / `page`） | **取引日は過去日でも記帳できる**（`entry_date` は取引日であって記帳日ではない。`CLAUDE.md` §7）。ページをまたぐ間に前のページより前へ挿入される仕訳がありうるので、オフセットは1つずれ、**その瞬間に行が黙って読み飛ばされる**。会計データの検索で「黙って1件消える」のは、無言の truncation と同じクラスの事故である |
| 上限を設けず全件返す | 大きな帳簿で応答が壊れる（`docs/07-mcp-server.md` §10 の要件）。壊れ方は「MCP クライアントが応答を切る」なので、**AI からは正常に見える**のが最悪である |
| 上限を超える `limit` を上限に丸めて成功させる | 呼び出し元は「要求した件数だけ返ってきた」と読む。`total_matches` があるので実害は小さいが、**丸めた事実そのものは応答から読み取れない**。上限を名乗って拒否すれば、次の手（`limit` を下げる／`cursor` で続きを取る）がそのまま分かる |
| 壊れたカーソルを無視して先頭から返す | 「続き」のつもりの要求が1ページ目を返し、呼び出し元は同じ内容を続きとして読む。無限ループにも化ける |
| 総件数を返さず `has_more` だけにする | 「あと何件あるか」が分からないと、AI は条件を絞るべきか続きを取るべきかを判断できない。総件数は同じ SQL 文の中で1回だけ数える（`total` の CTE に `page` を `LEFT JOIN` する形。ページが0行でも総件数が消えない） |
| ページ内の行から合計を組み立てさせる（合計を返さない） | AI がページの行を足して「期間合計」として使う。切れていれば必ず誤った合計になり、**それが誤りだと気づく手段が応答に無い** |

**理由**: 上限は「応答が壊れないため」に要るが、**上限そのものより
「切ったことが分かるか」の方が重要**である（`PROGRESS.md` の教訓
「無言の truncation は『全部見た』と読める」）。切れたことが分かれば
AI は続きを取るか条件を絞るかを選べるが、分からなければ**部分集合を全体として
扱った推論**が始まり、それは記帳内容の誤りに直結する。

keyset を選んだのは、この帳簿が append-only でありながら**過去日への追記が
起きる**という性質による。append-only だけならオフセットでも安全だが、
取引日順に並べる以上、後から前へ挿入されうる。

**ページングの検査は `limit = 1` だけでは効かない**（PR-H レビュー2巡目）。
1件ずつ辿るとページの**先頭と末尾が同じ**になるため、
「次のカーソルはページの**末尾**」を「先頭」に書き換えても結果が変わらない
（`rows.last()` → `rows.first()` にしても全テストが緑のまま通った。実測）。
先頭をカーソルにすると次のページが1件ずつしか進まず、**同じ行を何度も返す
終わらないページング**になるが、それが現れるのは `limit >= 2` のときだけである。
そこで差分テストは `limit = 1 / 2 / 3`、e2e は `limit = 1 / 2` で辿り、
**同じ仕訳に明細が2行ある科目**（カーソルの4項目の `line_no` が効く形）でも回す。

あわせて**各ページの行数が `limit` 以下**であることも見る。
続きの有無を判定するために `limit + 1` 行を取り `take(limit)` で切っており、
...
要らないと判断した。カーソルの形式は `kaikei-app` の関数2つを直せば変えられる
（線上に出るのは呼び出し元がそのまま返すだけの文字列である）。

---

## D-090 Phase 3 の完了条件は「ツールごとの検査」ではなく「1本の通し」で確かめる

**決定**: `ROADMAP.md` Phase 3 の4つの完了条件を、**AI が実際にやる一連の流れ**を
実バイナリに stdio で1本通すテストで確かめる
（`crates/kaikei-e2e/tests/mcp_walkthrough.rs`。PR-I で新設）。
...
1. **「全11ツールを踏んだこと」の一覧をテスト側に手で書かない。**
   通しの最後で `audit_log` に現れたツール名の集合と
   `kaikei_mcp::server::registered_tool_names()` を突き合わせる。
   ツールを1つ足してこの通しで呼ばなければ落ちる。
2. **自己修正は「エラーの形」ではなく「直して通ること」で見る。**
   1回目を `auto_tax_lines` 無しで送り、返ってきた
   `hint.suggested_lines` に**従って送り直して成功させる**。
3. **訂正済みであることは「応答が変わること」で見る。**
   訂正前後の `get_entry` の本文が**異なる**ことと、`reversed_by` が
   赤伝の仕訳IDを指すことを同時に主張する。
4. MC-01〜MC-12 の担い手を1件ずつ確認し、**実バイナリ経由で通っていな
   かったもの**（MC-04 / MC-05 / MC-09 / MC-10）を足す。対応表は
   `docs/07-mcp-server.md` §10 の冒頭に置く。
   **対応表の列は「実バイナリ経由（stdio）」と「直接呼び出し・単体」であり、
   その区別こそが表の存在理由である。** 実バイナリを子プロセスで起動する
   テスト（`mcp_stdio_server.rs` / `mcp_walkthrough.rs`）を右列に置かない
   ——置いた瞬間に、表が「どちらで通っているか」を答えなくなる
   （PR-I レビュー D-4。MC-11 の行が該当していた）。
5. 実バイナリの起動と stdio の読み書きは
   `crates/kaikei-e2e/tests/common/mcp_stdio.rs` に**共有**する
   （`mcp_stdio_server.rs` から移設）。

**却下した選択肢**:

| 候補 | 却下理由 |
|---|---|
| ツールごとの検査が11件揃っているので通しは要らない | **ツールをまたぐところで壊れる。** `get_entry` が訂正済みかどうかを返していなかった欠陥（PR-G レビュー B）は、ツール単体の応答としては正しく見えていた。壊れていたのは「`reverse_journal_entry` の後に `get_entry` を呼ぶ」という流れである |
| 通しも `dispatch::call` を直接呼ぶ形で書く（速い） | それでは `rmcp` のトランスポート・`ServerHandler`・ルータを通らない。Phase 3 の完了条件は「**Claude Code から**記帳できる」であり、確かめたいのは**その経路**である。実際 MC-09（金額を JSON number で渡す）は経路が本質的に違い、`Parameters<T>` に戻すとこの呼び出しだけが `audit_log` に残らなくなる |
| 踏んだツールの一覧をテスト側の定数にする | `PROGRESS.md` Phase 1 の教訓6「手で維持する一覧は必ず腐る」。Phase 4 でツールを足したとき、定数を直し忘れれば**そのツールは通しで一度も呼ばれないまま緑**になる |
| 帳簿の内容を1件だけにする（速い） | 「間違いに気づく」が表現できない。誤記帳 → 赤伝 → 記帳し直し、の**3件が同じ科目に並んだ状態**でないと、元帳の残高の推移も `reversed_by` の向きも検査にならない |
| ハーネスを複製する（`tests/*.rs` は別のコンパイル単位なので複製が自然） | 複製するのは「起動時の必須12項目」「stdout を JSON として読む」「実行ファイルがソースより古くないこと」——**壊れると検査が黙って無意味になる**部分である。片方だけ直したときに「古いバイナリに対して緑」が片側で復活する |

**理由**: Phase 3 の完了条件は4つとも**振る舞い**として書かれている
（「記帳できる」「自己修正できるエラーが返る」「全操作が記録される」
「削除系ツールが存在しない」）。それを11個の単体検査の**論理積**として
確かめると、個々は緑なのに**繋いだ瞬間に成立しない**状態を見逃す。

これは D-084 の4巡目で学んだことと同じ形である——走査（書き方の検査）が
3巡続けて破られ、**実バイナリに `tools/call` を送る振る舞い検査**に
担い手を移して収束した。今回はその単位を「1回の呼び出し」から
「ツールをまたぐ一連の流れ」へ広げただけである。

**トレードオフ**: 通しは1本で多くを主張するので、落ちたときに
**どこが壊れたのかが即座には分からない**。そこで既存のツールごとの検査
（`mcp_write_tools.rs` / `mcp_search_ledger.rs` / `mcp_stdio_server.rs`）は
そのまま残す。通しは「繋がっていること」だけを担い、細かい応答の形は
あちらが持つ。実行時間は約15秒（3テスト。使い捨てDBの作成と
サーバー起動を各テストで行うため）。

---

## D-091 README は「未経験の利用者が README だけで動かせる」を基準にし、書いた手順を実行して確かめる

**決定**:

1. README の先頭に**上から順に実行すれば動く**節（「5分で動かす」）を置く。
   前提（Docker / Rust）→ `.env` → マイグレーション → 単体起動の確認 →
   `.mcp.json` → 動作確認 → **うまくいかないときの表**、の順。
2. **`.mcp.json` の実物を載せる**（Claude Code から接続する形）。
   DB パスワードが平文で置かれることと、コミットしないことを併記する。
3. **できないことを独立した節にする。** ツールが無いもの（Phase 4 以降）・
   会計として実装していないもの・仕様上の制約（`tags` の重複キー、
   認証機構が無いこと、`accounts.active` が効かないこと等）を表で並べる。
4. **書いた手順を、書いた本人が最初から実行する**（PR-I では使い捨ての
   データベースを作り、終わったら `DROP` した）。
5. 起動失敗メッセージが名指しする **README の節名**を定数にし
   （`crates/kaikei-mcp/src/config.rs` の `README_SECTION`）、
   README の見出しと突き合わせるテストを置く
   （`the_readme_section_named_by_the_failure_message_exists`）。
   **誘導元は1つではない。** `.env.example` も本文で README の節を名指しして
   いるので、そちらも同じ検査に含める
   （`the_readme_sections_named_by_the_env_example_exist`。`.env.example` から
   `README「…」` の参照を**全部拾って**突き合わせる形にし、定数に1つだけ
   書き写さない——参照が増えたときに漏れるのがこの事故の形そのものである）。
6. **手順に必要な環境変数を省かない。** `kaikei-store` は `sqlx` の `query!`
   マクロを使うので、ビルドは既定で `DATABASE_URL` の DB を見に行く。
   マイグレーション前の DB にはテーブルが無いため、README の手順どおりに
   `cargo run -p kaikei-store --bin kaikei-migrate` すると
   `relation "accounts" does not exist` で**ビルドが落ちる**——
   「マイグレーションするバイナリのビルドに、マイグレーション済みの DB が要る」
   という循環である。`SQLX_OFFLINE=true` を手順に書き、**なぜ要るのかも書く**
   （`.github/workflows/database.yml` は同じ循環を知っていて
   `sqlx migrate run` を使っていたが、README にだけ回避が書かれていなかった）。
7. **`.env.example` は事業者設定に値を入れて配らない**（`#` でコメントアウト
   する）。`cp .env.example .env` して `CHANGE_ME` を置換しただけで起動できると、
   利用者は自分の帳簿が税抜経理か・課税事業者かを**一度も宣言しないまま**
   記帳を始める。D-057 / D-082 が「既定値にフォールバックしない」と決めた意味が、
   **配り方の側で骨抜きになる**。サーバーは不足項目を全部まとめて名指しして
   止まるので、思想どおり人間が決めることになる
   （検査は `the_env_example_ships_the_business_settings_commented_out`。
   接続文字列 `APP_DATABASE_URL` は事業者設定ではなく、かつ `CHANGE_ME` を
   置換しないと使えないので対象外）。
8. **README が作らせるファイルの後始末を利用者の作業として残さない。**
   手順4 はリポジトリ直下に `.mcp.json`（DB パスワードが平文で入る）を
   作らせる。`.gitignore` に入れるのはリポジトリ側の仕事である
   （`.env` について「`.gitignore` 済みです」と言えているのと同じ形にする）。
   このリポジトリの `.gitignore` には worktree について
   「`git add -A` で誤ってコミットされる（実際に一度踏んだ）」という注記が
   既にある。
9. **「できないこと」に書く数字は実測する。** 読み取り系の `audit_log.output` が
   要約になっているのは7件中2件（`search_entries` / `get_ledger`）であり、
   README にはそのとおり書く。「読み取り系の記録内容は要約です」と書くと、
   教訓3で消したはずの**提供していない保証**が1件復活する
   （同じ PR の `PROGRESS.md` は「★未消化★」と正しく書いており、
   文書間で食い違っていた）。
10. **訴求点で税務判断を断定しない**（`CLAUDE.md` §10）。「優良な電子帳簿の
    要件を満たすと 65万円の青色申告特別控除が得られる／このプロジェクトの
    価値は 10万円の控除差」は、複式簿記・期限内申告・e-Tax 等の別要件を
    落としている。**控除の可否は利用者と税理士が判断する**ことを明示し、
    書いてよい範囲（「〜の機能要件を意識した設計」）に留める。同じ文書の
    「できないこと」に**青色申告特別控除を実装していない**と書いてあるのだから、
    なおさら価値をその控除額で表現しない。
...
| 候補 | 却下理由 |
|---|---|
| 手順を書いて終わりにする（実行は利用者に任せる） | `PROGRESS.md` Phase 1 の教訓7がこれで起きた（`.env.example` の `DATABASE_URL` が `kaikei_app` を指しており、README どおりに設定した開発者は pg-tests が全滅した）。**書いた手順は書いた本人が一度実行する** |
| 「できないこと」を各節に散らす | 利用者が**着手前に**知りたいのはここである。散らすと、動かしてから「これはできない」に気づく。とくに `tags` の重複キー（後勝ちで税率が無言で入れ替わる）は、知らずに使うと帳簿が壊れる |
| できないことを「未対応」「今後対応予定」と書く | 予定を約束にしない。**いつ実装するかは決まっていない**ものは Phase を書くに留める |
| 起動失敗メッセージから README への参照を消す（節名の保守を避ける） | 12項目を1つずつ潰させないための誘導であり、消すと `CLAUDE.md` §11 に反する。**参照を保つコストは、テストで機械的に払う** |
| 節名の一致を「気をつける」で守る | `PROGRESS.md` Phase 1 の教訓6。実際に PR-I で README を書き直した際、メッセージが案内する節（「MCP サーバーを起動する」）は消えていた |
| 節名の検査は起動失敗メッセージ（`README_SECTION`）だけでよい | 誘導元がもう1つある。**利用者が最初に開くファイル**（手順1 の `cp .env.example .env`）が「README『ローカル開発環境』を参照」と書いたまま残っていた（PR-I レビュー B-5）。検査の対象を「メッセージ」ではなく「README を名指しする文書」で括る |
| `.env.example` に事業者設定の値を入れておく（`cp` してすぐ動いて親切） | **一度も宣言していない前提で帳簿が始まる。** 起動を止める側（D-057 / D-082）と配る側で言っていることが逆になる（PR-I レビュー C-2） |
| `.mcp.json` の `.gitignore` 追加は利用者に任せる | 平文のパスワードを置けと指示した側が、コミットされない保証を渡さないのは非対称。`.env` については「`.gitignore` 済みです」と言えている（PR-I レビュー C-3） |
| README に `SQLX_OFFLINE=true` を書かず「ビルドが落ちたら調べてもらう」 | 「この節は上から順に実行すれば動く」が成立しなくなる。しかも落ち方（`relation "accounts" does not exist`）は**原因と無関係に見える**（PR-I レビュー B-1） |
| 「読み取り系の記録内容は要約です」と簡潔に書く（細かい内訳は開発者向け） | 7件中5件は本文がそのまま入る。**要約されている前提で機微な検索語を投げる**利用者が出る。`audit_log` は append-only で消せない（PR-I レビュー B-2） |
| 訴求点は「控除差10万円」と書いておく（分かりやすい） | 帳簿の作り方だけでは決まらない。`CLAUDE.md` §10 が禁じる**税務判断の断定**に最も近い（PR-I レビュー D-1） |
| 記帳の手本を自然文だけにする（入力の形は `docs/07-mcp-server.md` にある） | README の手本どおりに頼むと `missing_required_tag` で拒否される。**収益・費用の明細に `tax_category` が必須**であることが README のどこにも無かった（PR-I レビュー C-1 / B-4）。README は「未経験の利用者が README だけで動かせる」が基準である |

**理由**: Phase 3 の完了条件「**Claude Code から**記帳できる」は、
ソフトウェアが動くことだけでは満たされない。**利用者が繋げられて初めて
満たされる**のであり、その手順書は成果物の一部である。

`.env.example` に必須項目が載っていることは既にテストで守っていたが、
守っていたのは**項目名**だけだった。手順の順序・節の名前・
「ここに一次情報がある」という参照そのものも、腐れば同じ実害を出す。

**トレードオフ**: README が長くなる（利用者向けと開発者向けが1つのファイルに
同居する）。分割は将来の選択肢として残すが、Phase 3 時点では
「1ファイルを読めば全部ある」ことを優先した。分割するなら
`docs/` ではなく `README.md` からリンクする形にする——起動失敗メッセージが
README を一次情報として名指ししているため。


---

## D-092 税区分マスタのファイル名は「暦年」ではなく「施行日」にし、期間の穴を検査で塞ぐ

**決定**:

1. `kaikei-jp-data/tax/jp/` のファイル名を**施行日**にする（`2026.yaml` →
   `2026-01-01.yaml`）。消費税の改正は暦年の途中に施行されるため、暦年を
   名前にすると「1つの暦年に2つのマスタが要る年」を名前で表現できない。
   D-050 が「暦年 → マスタ1件」を却下して適用期間で選ぶ形にした判断の、
   **ファイル名の側の対応**である。
2. **改正が来たら、施行日を名前にしたファイルを追加し、直前のファイルに
   `applies_to` を入れる。** 「既存ファイルは編集しない」という当初の
   運用（旧 `2026.yaml` のコメント）は撤回する。直前のファイルを閉じないと
   期間が重なり、`TaxRuleSets::new` が D-054 で構築を拒否する。
3. 令和8年度税制改正を反映し、免税事業者等からの課税仕入れに係る経過措置の
   控除割合を **2026-10-01 から 70%** にした（`2026-10-01.yaml`）。
   従来の想定（80% → 50%）は改正で 80% → 70% → 50% → 30% → 0% の5段階に
   変わっている。出典と全期間の表は `docs/09-tax-research.md`
   （国税庁インボイスQ&A 問113、財務省「令和8年度税制改正の大綱」）。
   割合の判定は**課税仕入れを行った日**で行う（同 問113-3。支払日でも
   請求書日付でもない）。これは `CLAUDE.md` §7「年度別データの選択は取引日で
   行う」と一致しており、追加の実装を要さない。
4. 欠落していた `PURCHASE_8_REDUCED_NON_QUALIFIED`（軽減8%・非適格）を
   両マスタに追加した。経過措置は軽減税率にも及ぶ（同 問113）。
5. **マスタ間で税区分コードの集合が一致していること**を検査する
   （`crates/kaikei-jp/tests/tax_master_timeline.rs`）。集合が食い違うと、
   **同じ記帳が取引日によって成功したり失敗したりする**。
6. **期間に穴が無いこと**を、最初のマスタの開始日から10年先まで**1日ずつ
   全部引いて**検査する。標本ではなく全数にしたのは、穴が月末
   （`applies_to` を1日ずらした場合の 9/30 など）に空くためで、日を絞った
   標本検査では踏み損ねる。
7. 検査に**期待値を手で書き写すのは1点だけ**にする——施行日
   （2026-10-01）と控除割合（0.80 / 0.70）である。ここは YAML から取った値を
   YAML と比べても何も守れない、制度の事実そのものだからである。
   同梱マスタの一覧も税区分コードの一覧も書かず、`TAX_CATEGORY_SOURCES` と
   マスタ同士の突き合わせから導く（D-047 / D-051 と同じ方針）。

**却下した選択肢**:

| 候補 | 却下理由 |
|---|---|
| `deduction_ratio` を期間つきの配列（スケジュール）にして1ファイルに収める | 適用期間は**マスタ全体の性質**であり、D-050 がそう決めている。区分ごとにスケジュールを持たせると、日付から値を決める経路が `TaxRuleSets::for_date` と区分内スケジュールの**2つ**になる。2つある以上いつか食い違い、しかも食い違ったときに気づく手段が無い |
| 暦年ファイルのまま `2026.yaml` に 70% を書く | 2026-01-01 〜 09-30 の課税仕入れに誤った割合が出る。改正日は暦年の途中である |
| 「年度途中の改正は翌年から反映する」と運用で決める | 制度と合わない。経過措置の割合は施行日から変わる |
| テストに全期間（80/70/50/30/0）の表を持たせる | まだ同梱していない期間の値を、検査だけが知っている状態になる。**手で維持する表が増える**（D-047）。実在する境界1つに絞り、マスタを足すときに1行足す |
| マスタ間のコード集合の一致を検査しない（区分は自由に増減してよい） | 欠落は実際に起きた（`PURCHASE_8_REDUCED_NON_QUALIFIED` が長く無かった）。制度上、区分が特定の期間にしか存在しないこと（少額特例は令和11年9月30日で終わる）は起こりうるので、**そのときは検査を期間つきの表に置き換える**。単に緩めると意図しない欠落が二度と検出できない |

**理由**: 改正は暦年の途中に来る。マスタをファイルで分ける方式には
「分けたときに穴を空ける」という固有の失敗があり、それは構築時には何も
起こさず、その日の取引を記帳しようとして初めて `NoApplicableRuleSet` で
落ちる——**帳簿を付けている最中に、その日だけ**である。
構造（D-050）は既に正しかったので、必要だったのはデータと、
**足し方を間違えたらその場で落ちる検査**だけだった。

**トレードオフ**: 改正のたびにファイルが増え、変わらない区分も含めて
全体を書き写すことになる（差分ではない）。書き写しは欠落を生むので、
上の集合一致テストがその代償を払う。マスタが十数件に増えて書き写しの
コストが実害になったら、「共通部分 + 差分」の表現を検討する——ただし
そのときも**日付から値を決める経路を2つにしない**こと。

**未対応として残すこと**（`docs/09-tax-research.md` の実装 TODO）:

- **2028-10-01（50%）以降のマスタは同梱していない。** `2026-10-01.yaml` の
  `applies_to` は `null` なので、追加しないまま 2028-10-01 を過ぎると
  70% を使い続ける。上の検査は「穴」は見つけるが「**古い値を使い続けている**」
  ことは見つけられない（期間が開いている限り引けてしまう）。
- **少額特例（税込1万円未満・〜2029-09-30・100%控除）の区分は追加していない。**
  制度自体は確定しているが、適用可否が事業者の基準期間課税売上高（1億円以下）
  に依存し、それを保持する仕組みが `JpSettings` に無い。税区分だけ足すと
  **要件を満たさない事業者が選べてしまう**。D-082（事業者設定は明示必須）と
  同じ論点なので、設定項目の追加と併せて決める。
- **`deduction_ratio` は依然として税額計算に反映していない**（D-059 は維持）。
  控除できない部分を本体価額に含める処理が原則的取扱いであることは
  一次情報で確認できた（所得税消費税経理通達3の2、経過的取扱い(2)）が、
  消費税経理通達関係 Q&A が令和8年度改正（70%）を反映していないため、
  **反映の形が確認できるまで実装しない**。

---

## D-093 `get_statements` を公開し、Phase 3 の11ツールを12にする

**決定**:

1. **D-070 が確定した「11ツール」を12に広げる。** 増やすのは `get_statements`
   （貸借対照表・損益計算書）1件だけで、他の予約済みツールは延期のまま。
   `crates/kaikei-mcp/tests/forbidden_tools.rs` の `PHASE_3_TOOLS` と
   `docs/07-mcp-server.md` §2 を同じ PR で更新する。
2. **D-031 の延期理由は解けた。** 「`TrialBalance` は `kaikei-core` の外から
   構築できない」は事実だが、[`TrialBalance::from_entries`] は公開されており、
   `&JournalEntry` のイテレータさえあれば組み立てられる。足りなかったのは
   仕訳をドメインモデルで引く経路であり、`JournalRepo::list_entries_in_period`
   がそれを埋めた。**read model の DTO を経由する必要は無かった。**
3. **住み分けを固定する。** 画面に出す試算表は read model（`get_trial_balance`。
   SQL 集計）、**帳簿から計算し直すもの**は `Tx` を開いてドメインモデルを引く
   （`get_statements` / 今後の決算振替・帳簿出力）。同じ「試算表」でも経路が
   違うのは、後者が `StatementPolicy` / `ClosingPolicy` の入力型に縛られるため。
4. **様式（`JpStatementPolicy`）はツールの中で都度組み立てる**（D-069 の維持）。
   そのため勘定科目表を同一トランザクション内で2回読む。読み直しを1回にするには
   ユースケースの引数で受け渡す形になるが、それは「呼び出し側が正しい勘定科目表を
   渡す」責任を増やすだけで、D-069 が避けたい事故（起動時に固めた古い科目名が
   決算書に出る）には効かない。
5. **貸借対照表が期首残高を欠く可能性を、応答で伝える。** `entry_count` と
   `first_entry_date` を常に返し、加えて次のいずれかで `balance_sheet_note` を
   添える:
   - `from` が会計年度の開始日（暦年なので 1/1）でない
   - `from` は年度開始日だが、最も古い仕訳がそこから離れている
   注記は**どちらも「疑わしい」までしか言わない**。実際に期首残高が要るかは
   帳簿と事業の状況によるので、断定すると `CLAUDE.md` §10 に触れる。
6. **読み取り系なので `audit_log.output` は要約にする**（D-089 決定6）。
   区分ごとの `lines` を落とし、小計・合計・注記は残す。要約は**応答本文から
   落として作る**（別に組み立てると値が食い違う）。

**却下した選択肢**:

| 候補 | 却下理由 |
|---|---|
| 貸借対照表を出すとき、`from` を会計年度の開始日に**自動で補正する** | 呼び出し側が指定した期間を黙って変えることになる。損益計算書は指定どおりの期間で返るので、**1回の応答の中で2つの表が別の期間を指す**ことになり、読み手はそれに気づけない |
| 年度途中から始まる期間では**エラーにする** | 期中の増減を見たいという正当な用途がある（月次の推移を追う等）。使えなくするのではなく、何が落ちているかを伝える |
| 注記を `warnings` キーで返す | dispatch 層が fail-open の注記のために予約している。`debug_assert!` が毎回検査するので `cargo test` が落ちる（Phase 3 の申し送り） |
| `explain_balance` も同時に公開する | D-031 を延期理由に挙げている点は同じだが、**残高の内訳を説明する**のは別の設計（どの粒度で何を根拠として出すか）が要る。1つの決定に2つの論点を混ぜない |
| 応答から `entry_count` を省く（`sections` を見れば分かる） | 0 件の期間でも**貸借一致した空の決算書**が成功で返る。空が「帳簿が空」なのか「期間の指定ミス」なのかは、金額を見ても区別できない（`PROGRESS.md` Phase 1 の教訓3） |

**理由**: Phase 5 の完了条件は「自分の確定申告に使える」であり、青色申告決算書の
中身は損益計算書と貸借対照表である。`get_statements` はその手前の最後の1段に
当たる。D-031 が延期理由として挙げた制約は、Phase 3 の時点では正しかった
（read model しか無かった）が、**ポートが1本増えたことで前提が変わっていた**。

**トレードオフ**: `get_trial_balance` と `get_statements` で、同じ帳簿に対して
別々の集計経路が走る。両者の数字が食い違ったらどちらかにバグがあるが、
**その突き合わせは今のところ自動では行っていない**。`kaikei-e2e` で
「同じ期間の試算表と財務諸表が整合する」ことを見る検査は、決算振替を公開する
段で入れる（そこで両方を通しで使うため）。

---

## D-094 決算振替は「提案」までを MCP に出し、締め（`close_period`）とは分ける

**決定**:

1. `propose_closing_entries` を公開する（許可リストは13件に。D-093 に続く追加）。
   返すのは仕訳の**案**であり、帳簿には何も書かない。
2. **応答に `posted: false` を必ず入れる。** 条件によって省いたりしない——
   「有るときだけ出す」形にすると、無いことに意味があるのか単に忘れたのかが
   読めない。**「提案が返った＝決算が済んだ」が最も危険な読み違い**であり、
   決算が済んでいないことに気づかないまま確定申告に進むと帳簿と申告が食い違う。
3. **集計期間は会計年度ラベルから導出し、日付では指定させない。**
   `from`/`to` を受け取れると、年度と1日ずれた期間で決算振替を提案できてしまう。
   それを記帳すると、ゼロ化しきれなかった収益・費用が翌年度に残る——
   **しかもその誤りは決算書を見ても分からない**（貸借は一致したままになる）。
4. **`close_period`（締め）とは分ける。** こちらは仕訳を提案するだけで、
   `period_snapshots` への記録も以後の記帳の拒否も行わない。`close_period` は
   Phase 4 以降のままである（checksum の計算式と canonical JSON の定義が
   Phase 5 の `kaikei verify` と揃っている必要がある。D-070）。
5. **提案が空のとき、2つの理由を書き分ける。** 「帳簿にその年度の仕訳が無い」と
   「既に決算振替が記帳されている」では次にすることが違う（前者は記帳から、
   後者は何もしなくてよい）。`entry_count` と `next_step` の文言で区別する
   （`CLAUDE.md` §11）。
6. **明細は `post_journal_entry` の入力にそのまま渡せる形で返す。**
   呼び出し側が組み替える必要があると、そこで写し間違いが起きる。

**却下した選択肢**:

| 候補 | 却下理由 |
|---|---|
| 提案と同時に記帳する（`post: true` オプション） | 決算振替は年度に1回の不可逆な操作で、間違えると赤伝で訂正するしかない。`suggest_tax_category` が候補を返すだけなのと同じく、**確定は人間に残す**（D-087 / `CLAUDE.md` §10） |
| 提案が空のときエラーにする | 決算が済んでいる年度に対して呼ぶのは正常な操作である。空であること自体が答えになる |
| `posted` を省き、記帳していないことは説明文に書くだけにする | 説明文は最初に1回読まれるだけで、**応答を見た時点では手元に無い**。読み違いの代償が大きいものは応答自体に入れる |
| 集計期間を `from`/`to` で受け取る（`get_statements` と揃える） | `get_statements` は「見るだけ」なので期間がずれても実害は読み手の誤解に留まるが、こちらは**記帳の材料**である。同じ形にする理由が、そのまま採らない理由になる |
| `close_period` も同時に実装する | 前工事（`period_snapshots` の checksum、canonical JSON の定義）が残っており、しかもそれは Phase 5 の `kaikei verify` と揃える必要がある。1つの決定に2つの論点を混ぜない |

**理由**: Phase 5 の完了条件「自分の確定申告に使える」には決算処理が要る。
`ClosingPolicy` の実装（`JpSoleProprietorClosingPolicy`）は Phase 2 から
あったが、**MCP から呼ぶ経路が無かった**ので、AI は決算振替を提案できなかった。
`list_entries_in_period` が入ったことでその経路が繋がった。

**トレードオフ**: 提案を記帳するかどうかが呼び出し側に委ねられるので、
「提案しただけで満足して記帳し忘れる」経路が残る。`posted: false` と
`next_step` で伝えてはいるが、**帳簿の側から「この年度は決算振替が未記帳である」
ことを検出する仕組みは無い**。それは `close_period`（締め）が担うべき役割で、
Phase 4 以降の宿題として残る。


---

## D-095: `search_documents` を MCP に載せる（証憑の中身は返さない）

**決定**: 証憑をメタデータで探す `search_documents` を MCP ツールとして
登録する。**ファイルの実体は返さない。**

| 案 | 採否 | 理由 |
|---|---|---|
| メタデータだけ返す | **採用** | 応答の量が証憑の件数にしか比例しない |
| 実体（base64）も返せるようにする | 却下 | 量がファイルの大きさに比例し、append-only の `audit_log` に毎回残る（§9）。PDF 数MBが監査ログに積み上がる |
| 実体を返す別ツールを足す | 却下 | 同じ問題。実体の取り出しは `kaikei report` が `documents/` へ書き出す経路で足りる |

**理由**: 日々の運用で AI が答えたいのは「この取引の証憑はあるか」であって、
中身を読むことではない。内容の SHA-256 を返すので、保存先のどのファイルかは
特定できる。

**トレードオフ**: AI から証憑の中身を確認する経路が無い。必要になったら
`kaikei report` で書き出してから読むことになる。**書き出しは中身の検証を
伴う**（変わっているものは出さない）ので、遠回りに見えて安全側でもある。

**前提の変化**: `docs/07-mcp-server.md` §2 はこのツールを「Phase 4 以降・
延期理由: `kaikei-blob` 未着手」としていた。`kaikei-blob` と
`documents` / `entry_documents` を実装したので、その理由は解けた。

---

## D-096: `list_pending_transactions` を MCP に載せる（明細を仕訳の形にしない）

**決定**: 取り込んだ明細のうち未仕訳のものを返す `list_pending_transactions` を
MCP ツールとして登録する。**返す形は明細のままにし、仕訳の語彙に寄せない。**

| 案 | 採否 | 理由 |
|---|---|---|
| 明細のまま返す（`is_money_in`） | **採用** | 取込データは仕訳ではない（`docs/05-csv-import.md` §1）。科目が決まるのは仕訳化のとき |
| 借方/貸方に翻訳して返す | 却下 | どちらの側にどの科目が立つかは、まだ決まっていない。先取りすると AI が「もう決まっている」と受け取る |
| 仕訳の候補まで含めて返す | 却下 | 提案はルール（`kaikei_app::journalize`）と AI の判断で別に行う。読み取りツールに混ぜると、根拠のない候補が既定値として通る |

**理由**: 明細と仕訳を同じ形で返すと、「入金＝貸方」のような取り違えが AI 側で
起きる。返金の入金は費用の取消であり、貸方に費用が立つ。**向きと側は1対1では
ない。**

**0件の扱い**: 空配列を返しエラーにしない。ただし状態ごとの件数（`counts`）を
添える。**「未処理が0件」には2つの意味がある**——全部片付いたのか、そもそも
1件も取り込んでいないのか。前者は喜ぶところだが、後者は CSV を流し忘れて
いるということで、確定申告の直前にこれを取り違えると帳簿に丸ごと抜けができる。
`search_documents` の `total_registered` と同じ考え方（D-095）。

**惜しい指定を通さない**: `status` の語彙は `pending` / `journalized` /
`ignored` に限り、`Pending` のような指定はエラーにする。`ImportedTxQuerySpec`
は文字列を素通しするので、検査しないと黙って0件になり「未処理は無い」と
読み違える。

**トレードオフ**: 明細を仕訳にする経路は MCP にまだ無い。記帳は
`post_journal_entry` を別に呼び、取込明細の状態を進めるのは今のところ
できない。**記帳と状態遷移は同じトランザクションで行う必要がある**
（片方だけ残ると二重計上の入口になる）ので、1つのツールとして別に足す。

**前提の変化**: `docs/07-mcp-server.md` §2 はこのツールを「Phase 4 以降・
延期理由: `kaikei-import` 未着手（crate もテーブルも存在しない）」として
いた。`kaikei-import` と `imported_transactions`（0011）を実装したので、
その理由は解けた。

---

## D-097: 明細の仕訳化は1つのツールにする（記帳と状態遷移を分けない）

**決定**: 取り込んだ明細を仕訳にする `journalize_transaction` を、記帳と
状態遷移の**両方を行う1つのツール**として登録する。

| 案 | 採否 | 理由 |
|---|---|---|
| 1つのツールで両方行う | **採用** | 同じトランザクションに収まる。片方だけ残る形が作れない |
| `post_journal_entry` の後に `mark_journalized` を別に呼ばせる | 却下 | 2回の呼び出しを1つのトランザクションに束ねる方法が MCP に無い |
| `post_journal_entry` に `imported_tx_id` を足す | 却下 | 明細と無関係な記帳（減価償却・決算整理）にも取込の語彙が付いて回る |

**理由**: 間で落ちると、**帳簿に仕訳だけが残って明細は未処理のまま**になる。
次にもう一度仕訳化すれば二重計上であり、帳簿は追記のみなので手で逆仕訳を
起こすまで金額がずれ続ける。**この失敗は決算書を見ても分からない**（貸借は
一致したままで、金額だけが二重になる）。

**取引日は明細から採る**: 既定では明細の取引年月日を使う。AI に日付を
組み立てさせると、打ち間違いが**別の年度の仕訳**になって現れる。ただし
`entry_date` での上書きは許す——カードの明細では引落日と購入日が違い、費用の
計上は購入日に寄せるのが正しい。

**金額の取り違えを見つける**: 明細が 19,800 円なのに 1,980 円で記帳しても、
貸借さえ合っていれば帳簿は受け取る。**桁の落ちた仕訳は決算書を見ても
分からない。** そこで「口座が動いた側の行に、明細と同じ金額が1行あるか」を
見る。

| 案 | 採否 | 理由 |
|---|---|---|
| 口座が動いた側の1行と突き合わせる | **採用** | 手数料差引き・家事按分・税抜経理のどれでも通り、桁落ちだけを止める |
| 仕訳の合計と突き合わせる | 却下 | 振込手数料が差し引かれた入金（入金 100,000 ＋ 手数料 10,000 ＝ 売上 110,000）が通らなくなる。**この記帳は普通にある** |
| 検査しない | 却下 | 桁落ちが見つかるのは確定申告のときになる |

**逃げ道を用意する**: それでも一致しない形はありうるので、
`allow_amount_mismatch` で意図的に通せるようにする。**既定では通さない**
——黙って通ると、桁落ちが見つかるのは1年後になる。

**トレードオフ（解消済み）**: 初版の `find_pending` は read model を200件
引いてから ID で絞っていた。「IDで1件引く」口が `ImportedTxQuery` に無かった
ためで、明細が200件を超えると古い側が引けなくなる（**IDを持っているのに
「見つかりません」になる**）。**帳簿が育つほど起きやすくなる失敗**なので、
`ImportedTxQuery::find_imported` を足して解消した。`suggest_journal_entry`
（D-098）も同じ口を使う。

---

## D-098: 仕訳の提案は「過去の自分の記帳」を根拠にする（ルールを MCP に持ち込まない）

**決定**: `suggest_journal_entry` は、**過去に同じような摘要でどう記帳したか**
を根拠として候補を出す。仕訳化ルール（`kaikei_app::journalize`）は使わない。

| 案 | 採否 | 理由 |
|---|---|---|
| 過去の記帳を根拠にする | **採用** | 帳簿から引ける。ルールを1本も書いていなくても効く。根拠が「自分が前にこうした」なので人が検証できる |
| ルール（YAML）を読んで当てる | 却下 | ルールの読み取りは CLI にある（`kaikei-mcp` は `serde_norway` を持たない。§10 の許可リスト）。MCP に持ち込むと許可リストを広げることになる |
| 両方を混ぜて返す | 却下 | 根拠の種類が混ざると、どちらの理由で提案されたかが読み取りにくい。ルールで確定できるものは CLI の `kaikei journalize` が見せる |

**理由**: 既存の会計ソフトも学習型の自動仕訳を持つが、**なぜその科目にしたかを
説明できない**（`docs/05-csv-import.md` §6）。ここが返すのは科目だけではなく、
**どの摘要で探して、過去のどの仕訳が当たったか**である。根拠が見えれば、提案が
外れているときに人が気づける。

**摘要は段階的に緩める**: 銀行の摘要には可変部分が混ざる
（`カ)アマゾン ジヤパン 12345` の番号）。全体で探すと1件も当たらないので、
当たらなければ**摘要から取り出した最も長い語**でもう一度探す。
**どちらで当たったかは隠さない**（`matched_by`）——緩めたことを隠すと、
たまたま当たった別の取引を「よく似ている」と読んでしまう。

**赤伝と取り消された仕訳は根拠にしない**: 取り消したということはその記帳が
誤りだったということで、繰り返す理由が無い。

**候補は仕訳の形にする**: 科目だけを頻度で並べると、借方と貸方が別々に選ばれて
成り立たない組み合わせができる。**金額は候補に含めない**——金額を含めると、
同じ記帳でも額が違うだけで別の候補になる。按分するかどうかは人が決める。

**断定しない**: `confidence` は件数と揃い方から決めた目安である。1件しか
無いものは揃っていても `high` にしない——最初の1回の誤りがそのまま増える。
`high` でも確定ではないことを、ツールの説明にも書く。

## D-099: 適格請求書が要る税区分に取引先が無ければ件数を出す（税務判断はしない）

**決定**: `kaikei report` と `kaikei verify` は、`requires_qualified_invoice`
が真の税区分を持つ明細のうち、`counterparty` タグが無いものの**件数**を出す。

| 案 | 採否 | 理由 |
|---|---|---|
| 件数だけを出す | **採用** | 帳簿の事実。仕入税額控除の可否は証憑の保存状況と相手方の登録状況で決まり、帳簿だけでは決まらない（§10 の「税務判断を断定しない」） |
| 記帳時に取引先を必須にする | 却下 | 過去の記帳が全部弾かれる。`counterparties` マスタは空で、登録する経路も無い（`PolicyError::UnknownCounterparty`） |
| 「控除できません」と出す | 却下 | 断定になる。帳簿に取引先が無いことと、適格請求書が無いことは別である |
| 何も出さない | 却下 | **`JpTaxPolicy` は取引先タグが有るときにしか適格性を見ない。** 無いまま記帳すると検証がすり抜ける。実際に weBanana.SP の帳簿は 603 件がこの状態だった |

**なぜ日付でマスタを選ばないか**: `requires_qualified_invoice` は施行日で
変わらない。日付で選ぶと、**期首仕訳のように会計期間の外に日付を持つ仕訳で
「該当マスタなし」になり、検査そのものが黙って飛ぶ**。数え落としが警告なしで
起きるので、コードを知っているマスタで判定する。

**verify にも入れる**: `verify` が `report` より弱いと、`verify` が通ったことを
根拠にできない。`warn_from_statements` は財務諸表しか読んでいなかったが、
税区分と取引先はタグなので仕訳そのものを読む必要がある。

## D-100 取引先マスタの投入は CSV から追加のみ。`is_qualified` の空欄は「未確認」であって「非適格」ではない

**決定**: `kaikei counterparty import --file <CSV> [--commit]` を作る。
既存行は**上書きしない**（`ON CONFLICT (code) DO NOTHING`）。CSV の
`is_qualified` 列は、空欄 = `NULL`（未確認）／`true`／`false` の3値とする。

| 案 | 採否 | 理由 |
|---|---|---|
| CSV から追加のみ | **採用** | `import_chart`（D-081）と同じ形。差分がファイルで見え、入れる前に下見できる |
| 外部システムの値で毎回同期する | 却下 | `is_qualified` は**ユーザーが確認したという記録**。同期すると確認結果が消える |
| MCP ツールで1件ずつ登録 | 却下（今は） | 記帳の流れの中で登録したくなったら足せばよい。まず一括で入れる経路が要る |
| 空欄を `false` として取り込む | **却下（重要）** | 下記 |

**なぜ空欄を `false` にしてはいけないか**: `JpTaxPolicy` は
`is_qualified_invoice_issuer == Some(false)` のときだけ記帳を拒む
（`QualifiedInvoiceUnverified`）。`None`（未確認）と `Some(false)`
（非適格だと確認した）は**別の意味**である。

外部システムの `false` は、多くの場合**誰も入力していないだけ**である。
実際に freee の取引先 34 件を調べたところ、`invoice_registration_number` は
全件 `null`、`qualified_invoice_issuer` は全件 `false` だった。これを
`Some(false)` として取り込むと、**34 社すべてを「非適格だと確認済み」に
仕立てることになる。**

**入力内の重複はエラーにしない**: 外部システムには表記ゆれの重複が普通に
ある（freee には「株式会社 ビーテック」と「株式会社ビーテック」が別々に
登録されていた）。挿入は `ON CONFLICT DO NOTHING` が飛ばすので落ちないが、
**件数の報告が実際と食い違わないよう**、ユースケース側で落として
`kept_existing` として見せる。

**既定は下見**（`kaikei import` と同じ）: 取引先コードは仕訳のタグに入る値で
あり、**一度タグを付けた後にコードを変えると、過去の仕訳が指す先が消える**
（`journal_lines` は追記型なのでタグを直せない）。

**既存の 603 件には後から付けられない**: `journal_lines` は追記型で
`UPDATE` を許していない。この経路が効くのは**これから記帳する分**である
（ユーザーの判断。逆仕訳＋再記帳で付け直す案は、取り消し理由が
「タグの付け忘れ」になり帳簿が倍に膨らむので採らない）。

## D-101 決算振替に `entry_kind: closing` の印を付け、決算書は常にそれを外して集計する

**決定**: `JpSoleProprietorClosingPolicy::closing_entries` が生成する**全明細**に
`entry_kind: closing` タグを付ける。`StatementsInput::exclude_closing` を足し、
決算書を出す経路（`kaikei report` / `kaikei verify` / `get_statements`）は
常に `true` を渡す。

**なぜ**: 決算振替は収益・費用をゼロにする。**記帳した瞬間から、その年度の
決算書は売上0・所得0になる。** 帳簿は追記型なので取り消せず、逆仕訳を切るしか
戻す手がない。しかもこれが起きるのは、決算書が最も要る時期（申告直前）である。

青色申告決算書の貸借対照表は「青色申告特別控除前の所得金額」を独立の行として
持つ——**所得を元入金へ振り替える前の姿**である。決算書を出す経路が決算振替を
外すのは、様式の側から見ても正しい。

| 案 | 採否 | 理由 |
|---|---|---|
| `entry_kind` タグで印を付け、決算書から外す | **採用** | 記帳の順番に関係なく決算書が同じになる。実帳簿の複製で検証済み（下記） |
| 摘要の文字列で見分ける | 却下 | 摘要は人が書き換えられる。手で切った決算振替には当てはまらない |
| 警告だけ出す（`warn_if_the_year_looks_closed`） | 却下（併存） | 気づくのは記帳した後。追記型なので気づいても戻せない。警告は印の無い決算振替への網として残す |
| 決算振替を記帳させない | 却下 | 総勘定元帳・仕訳日記帳には要る。帳簿としては正しい仕訳である |

**全明細に付ける**: 収益・費用のゼロ化だけでなく**元入金の明細にも付ける**。
1行でも漏れると、決算書から外したときに貸借が合わなくなる。

**`aggregatable: false`**: 用途は絞り込みであって集計ではない。`true` にすると
`get_trial_balance` の `group_by` に増え、MCP の公開面が広がる。

**試算表は外さない**: `trial_balance` は帳簿そのものを見るための表なので、
決算振替も含める。実際、複製で検証したとき変わったのは試算表だけだった。

**検証**: 実帳簿の複製（`kaikei_webanana` の TEMPLATE）に決算振替を記帳し、
`income_statement.csv` / `balance_sheet.csv` / `blue_return.csv` /
`blue_return_balance_sheet.csv` が**1バイトも変わらない**ことを確認した。

## D-102 期首振替（事業主貸・事業主借 → 元入金）は翌年1月1日に置き、`entry_kind: opening` で決算振替と区別する

**決定**: `JpSoleProprietorClosingPolicy::opening_entries` を実装する。
事業主貸・事業主借をゼロにし、差額を元入金へ振り替える仕訳を、
**翌年の1月1日**付で提案する。印は `entry_kind: opening`（決算振替の
`closing` とは別の値）。D-065 の「当年度末と翌年期首のどちらか未確定」を
この決定で置き換える。

**なぜ翌年期首か**: **青色申告決算書の貸借対照表に事業主貸・事業主借の欄が
ある。** 事業主貸は資産の部、事業主借は負債・資本の部に、それぞれ期末残高を
書く様式である。当年度末にゼロ化すると2つの欄が必ず0になり、様式と食い違う。
これは税務判断ではなく**様式から読み取れる事実**である。

**なぜ振替仕訳か（期首残高の直接設定ではなく）**: この帳簿は追記型で、
「期首残高を直接書き換える」経路を持たない。残高は仕訳の積み上げでしか動かない。

### `closing` と `opening` を分ける理由（役割が逆）

| 印 | 日付 | 決算書での扱い |
|---|---|---|
| `closing` | 当年度末 12/31 | **外す**（外さないと売上0・所得0になる） |
| `opening` | 翌年 1/1 | **外さない**（外すと事業主貸・事業主借が翌年度に残り続ける） |

最初は両方 `closing` にしていた。実帳簿の複製で2027年の決算書を出したところ、
期首振替を記帳済みなのに**事業主貸 10,013,438 が貸借対照表に残った**。

### 除外は日付で絞る（D-101 を更新）

D-101 は `exclude_closing: bool` で**すべての**決算振替を外していた。これでは
前年度の決算振替（前年12/31の所得→元入金）まで外れ、元入金が過少になり、
前年度の収益・費用が当年度の貸借対照表に漏れる。`exclude_closing_on:
Option<AccountingDate>` に変え、**その年度末の分だけ**外す。

### 期首の列には期首振替を含める

決算書の期首の列は「期首振替を済ませた後、その年の商売が始まる前」の姿である。
前日（前年12/31）で切ると期首振替が入らず、複製で確認したところ**期首の貸借が
合わなかった**（資産 10,121,943 に対し負債・資本 1,120,939）。
`only_opening_on` で「その日は期首振替だけを入れる」とした。

**期首残高の仕訳は前年末日付で入れること。** 1月1日に立っている仕訳が期首の姿
なのか初日の商売なのかは日付だけでは区別できず、区別できるのは
`entry_kind: opening` の印だけである。実帳簿もそうなっている
（「前期末日付で計上」）。

### 実帳簿の複製での検証

`kaikei_webanana` の複製に決算振替と期首振替の両方を記帳した。

| | 2026 | 2027 |
|---|---|---|
| 事業主貸 期末 | 10,013,438 | **0** |
| 事業主借 期末 | 1,012,434 | **0** |
| 元入金 | −1,814,261 | **−2,416,487** |
| 貸借対照表 合計（期首/期末） | 50,975 / 10,121,943 | 108,505 / 108,505 |

2026年の決算書は記帳の前後で1バイトも変わらない（D-101）。
2027年の元入金 −2,416,487 は手計算（−1,814,261 + 8,398,778 − 9,001,004）と一致する。

## D-103 減価償却は「耐用年数と償却方法を受け取って計算するだけ」にする

**決定**: `kaikei_jp::depreciation` は取得価額・取得日・償却方法・耐用年数・
事業専用割合を**入力として受け取り**、年ごとの償却費を返す。
**耐用年数を決めることも、どの償却方法を選ぶかを決めることもしない。**

```text
耐用年数・償却方法 ─→ [depreciation] ─→ 年ごとの償却費
  （人が決める）           （計算するだけ）
```

| 案 | 採否 | 理由 |
|---|---|---|
| 入力として受け取る | **採用** | 耐用年数の判定も償却方法の選択も申告上の判断であり、帳簿からは決まらない（`CLAUDE.md` §10） |
| 資産名から耐用年数を推定する | 却下 | 「パソコン」でもサーバー用は5年、それ以外は4年。中古なら簡便法。推定して黙って使うと、誤りに気づけないまま申告に載る |
| 取得価額から償却方法を決める | 却下 | 20万円未満・30万円未満は**選べる**のであって決まるわけではない。実際 108,000円 の資産には3つの扱いがあり、初年度の償却費は 45,000／36,000／108,000 と大きく違う |
...
| 一括償却資産 | 取得価額 × 1/3 を3年 | **しない** | 0円 |
| 少額減価償却資産 | 取得年に全額 | しない | 0円 |

**定率法は実装しない。** 個人事業主の法定償却方法は定額法で、定率法には
届出が要る。必要になってから足す。

**定額法は耐用年数より1暦年多くかかる。** 初年度を月割した分が後ろへずれる
（7月取得・耐用年数4年なら2025〜2029年の5暦年）。

**端数は切り捨て、最後の年で辻褄を合わせる。** 毎年切り捨てた分が積もると、
単純に並べただけでは合計が取得価額に届かない（100,000 ÷ 3 は 33,333 × 3 =
99,999 にしかならない）。

**事業専用割合は各年の償却費に掛ける。** 累計に掛けると端数処理の回数が
変わって合わなくなる。帳簿価額は按分前で追う（資産の未償却残高そのもの）。

### 実帳簿で検算した

2022-08-05 取得の「pc」118,800円 は、実際の帳簿で 2022〜2024年に 39,600円ずつ
償却されていた。**一括償却資産として計算すると1円まで一致する**
（`lump_sum_matches_the_real_book`）。8月取得でも初年度が満額なのが決め手で、
定額法なら月割で 5/12 になる。

## D-104 固定資産台帳は DELETE を持たない。資産を外すのは除却であって削除ではない

**決定**: `fixed_assets`（`0012`）は `UPDATE` を許すが **`DELETE` は与えない**
（DB 権限でも `REVOKE`）。資産を帳簿から外すのは `disposed_on` を埋めることで
ある。

| 案 | 採否 | 理由 |
|---|---|---|
| UPDATE 可・DELETE 不可 | **採用** | 耐用年数の見直しや事業専用割合の変更は起きる。一方、消せると**過去の年度の償却費がどの資産のものだったか辿れなくなる** |
| 帳簿本体と同じ append-only | 却下 | 状態が変わるたびに行が増え、「今どうなっているか」が読めなくなる（0011 と同じ理由） |
| DELETE も許す | 却下 | 上記のとおり |

**「計上済み」フラグを台帳に持たない。** 記帳したかどうかは帳簿
（`journal_entries`）が持つ。台帳にも置くと同じ事実を2箇所が持つことになり、
必ずずれる。

### 定額法かどうかで耐用年数の要否が変わることを CHECK で書く

| 制約 | 何を止めるか |
|---|---|
| `straight_line_needs_life` | 定額法なのに耐用年数が無い（計算時に落ちる。**入口で止める**） |
| `other_methods_take_no_life` | 一括償却・少額特例に耐用年数が入っている（**入れた本人は効くと思っている**） |

一括償却と少額特例は耐用年数を使わない（D-103）。入っていても無視されるだけ
なので、黙って受け取ると誤解が残る。

### `kaikei-app` は償却方法を数値で持つ

`kaikei-app` は `kaikei-jp` に依存できない（CI が禁じている）。台帳の行は
`method: i16`（1=定額法 / 2=一括償却 / 3=少額特例）で持ち、
`DepreciationMethod` への翻訳は端（CLI / MCP）が行う。

### `business_ratio` は境界で文字列にする

列は `NUMERIC(5,4)`（`0 < ratio <= 1` を CHECK で守る）。sqlx の
`rust_decimal` 機能を有効にしていないので、`SELECT business_ratio::text` と
`$N::numeric` で文字列として出し入れする。`kaikei-app` が `Decimal` を公開の
型に出さない方針とも合う。読むと `"0.8000"` の形になるが、
`Ratio::parse_fraction` はそのまま解釈できる。

## D-105 定額法の償却率は「1/耐用年数」ではなく小数第3位に切り上げた値を使う

**決定**: 定額法の年額は `取得価額 × 償却率` で求める。償却率は
**1/耐用年数を小数第3位に切り上げた値**（`1000_u32.div_ceil(n)` の千分率）。

### 当初の実装は誤りだった

D-103 の実装は `取得価額 ÷ 耐用年数` にしていた。減価償却資産の耐用年数等に
関する省令 別表第八の定額法償却率は切り上げなので、**割り切れない耐用年数で
食い違う**。
...
取得価額 100,000円・耐用年数3年なら、当初は 33,333円、正しくは 33,400円。

実帳簿の資産は2年と4年で、たまたま影響が無かった。**割り切れる耐用年数しか
試していなかったのが見落としの原因である。**

**2〜20年について別表第八と一致することを確かめた**
（`the_rate_matches_the_published_table`）。それより長い耐用年数は表と
突き合わせていない。

### 切り上げなので合計が取得価額を超える

償却率を掛けた額の合計は取得価額を超える（0.334 × 3 = 1.002）。
最終年で残額まで頭打ちにすることで辻褄が合う。D-103 で入れた
「残りを超えて償却しない」がそのまま効く。

### 償却率を `Schedule` に載せる

青色申告決算書の「減価償却費の計算」欄に**償却率を書く欄がある**。
計算に使った値をそのまま出せるよう `Schedule::rate_per_mille` で返す。
定額法以外は `None`（一括償却・少額特例は償却率を使わない）。

## D-106 決算書の「減価償却費の計算」欄は台帳から作り、帳簿と食い違えば知らせる

**決定**: `kaikei report` が `blue_return_depreciation.csv` / `.html` を
固定資産台帳から書き出す。列は様式の欄の並びに合わせる（転記する人が上から
順に写せるように、名前も順番も変えない）。

**使わない欄を 0 にしない。** 一括償却・少額特例は耐用年数も償却率も使わない。
`0` と書くと「耐用年数0年」と読める。斜線（`—`）で出す
（`blue_return_bs` の期首欄と同じ扱い）。

### 台帳と帳簿が食い違えば必ず言う

決算書には**台帳から出した額**を書く。帳簿の減価償却費と違っていると、
決算書と損益計算書で数字が合わなくなる。

```
注意: 固定資産台帳から出した償却費と、帳簿の減価償却費が違います。
  台帳（2 件）: 124,179 円
  帳簿の減価償却費: 0 円
  決算書には台帳の額を書くので、このままだと損益計算書と食い違います。
  台帳の入力・記帳の漏れ・記帳し過ぎのいずれかを確かめてください。
```

**どちらが正しいかは決めない。** 台帳の入力が違うのか、記帳が漏れているのか、
記帳し過ぎているのかは帳簿からは決まらない。事実だけを出す。

## D-107 固定資産台帳と帳簿の突き合わせは verify でも出す

**決定**: 台帳の期末簿価と帳簿の残高を**科目ごとに**比べ、食い違えば
`kaikei report` と `kaikei verify` の**両方**で知らせる。

**なぜ verify でも出すのか**: report を出したときにしか気づけないと、
年末まで食い違いが放置される。`verify` は月次の締め（`/kaikei-close`）で
毎月走るので、早く気づける。片方でしか出ないと、見ている方によって
気づけるかどうかが変わる。

**なぜ科目ごとか**: 合計だけを比べると、どの資産を疑えばよいか分からない。
実帳簿では3科目とも食い違っており、原因が科目ごとに違った。

### 実帳簿で出た指摘

```
注意: 固定資産台帳の期末簿価と帳簿の残高が違う科目が 3 件あります:
  205  台帳 0 円 / 帳簿 118800 円（差 118800）
  210  台帳 175449 円 / 帳簿 161917 円（差 -13532）
  220  台帳 9000 円 / 帳簿 108000 円（差 99000）
```

| 科目 | 何が起きているか |
|---|---|
| 205 機械装置 | 2022年取得の pc は2024年で償却し終えているのに、**償却の相手科目を取り違えていた**ため取得価額のまま残っている |
| 220 車両運搬具 | 2025・2026年の償却が**未計上**（45,000 + 54,000 = 99,000） |
| 210 工具器具備品 | 償却が未計上のうえ、過去のマイナス残高の打ち消しが混ざっている |

**貸借は一致したままなので、決算書を見ても分からない。**

**どれが原因かは決めない。** 償却の記帳漏れ・記帳先の取り違え・台帳の入力誤りの
いずれもありうる。事実（台帳と帳簿の額）だけを出す。

### まだ取得していない資産は数えない

取得年が対象年度より後の資産を数えると、**来年買う予定のものが今年の帳簿と
食い違って見える**。除却した年より後も同じく数えない。

## D-108 除却した年**以降**を台帳の計算対象から外す

**決定**: `disposed_on` が埋まっている資産は、**除却した年そのもの**から
償却額の計算・決算書の欄・帳簿との突き合わせのすべてで対象外にする。

| 案 | 採否 | 理由 |
|---|---|---|
| 除却した年から外す | **採用** | 除却した年の償却をどう扱うか（月割で計上するか、除却損に含めるか）は**申告上の判断**であり、帳簿からは決まらない。決めないので計算しない |
| 除却した**翌年**から外す | 却下 | 除却した年に月割で償却するのが正しいと決めたことになる。それは税務判断である |
| 除却しても計算し続ける | 却下 | 手放した資産の償却費が出続ける |

判定は `is_outside_the_ledger_for` の1関数にまとめた。3箇所（`depreciation`
コマンド・決算書の欄・帳簿との突き合わせ）で同じ規則を使う。

**取得前も同じ関数で外す。** 数えると、来年買う予定のものが今年の帳簿と
食い違って見える。

### `dispose` は未償却残高を必ず見せる

除却すると台帳の計算対象から外れるが、**帳簿にはまだ残高が残っている**。
除却損の記帳が要る。いくら残っているかを示さないと、記帳しないまま
`verify` の指摘だけが増える。

```
除却直前の未償却残高  63,000 円

この額が帳簿に残ります。除却損などの記帳は別に必要です。
  借方 <除却損などの科目> / 貸方 220
金額と科目は事情で変わる（売却なら売却損益）ので、こちらでは決めません。
```
...
`UPDATE ... WHERE disposed_on IS NULL`。除却日を後から動かすのは、
**過去の決算書の数字が変わる**ということである。投入経路が黙ってやってよい
ことではない（`ChartWriteRepo` が既存を上書きしないのと同じ規律）。

## D-109 減価償却費の仕訳は MCP で提案する（CLI からは記帳しない）

**決定**: `propose_depreciation_entries` を MCP に足す。固定資産台帳から
年度の償却費を計算し、`借方 減価償却費 / 貸方 それぞれの資産の科目` の1本に
まとめて返す。**記帳はしない**（`posted` は常に `false`）。

| 案 | 採否 | 理由 |
|---|---|---|
| MCP のツールにする | **採用** | 提案は MCP から出す形が既にある（`propose_closing_entries` / `suggest_journal_entry`）。記帳も MCP（`post_journal_entry`）なので、提案から記帳まで同じ経路で完結する |
| CLI が仕訳の JSON を出す | 却下 | 人が手で MCP へ写すことになる。**転記のために JSON を目で追う**のは、いま手で金額を写しているのと変わらない |
| CLI から記帳する | 却下 | 記帳は MCP だけの仕事である（`kaikei journalize` も「記帳するには MCP の post_journal_entry を使います」と案内している）。CLI に記帳経路を作ると、監査ログの入口が2つになる |

**台帳の値をそのまま使う。** 耐用年数も償却方法も推定しない（D-103）。

**空の理由を区別できるようにする。** 台帳が空なのか、その年度に償却する
ものが無い（取得前・償却済み・除却済み）のかで次にやることが違う。
`asset_count` を添える。

**`kaikei depreciation` は残す。** 記帳する前に額を見るだけの用途があり、
CLI の方が手軽である。両方が同じ `kaikei_jp::depreciation` を使うので
数字は一致する。

## D-110 マイグレーション未適用（SQLSTATE 42P01）を `Backend` に混ぜない

**決定**: `42P01`（undefined_table）を `RepoError::SchemaOutOfDate` に写像し、
「マイグレーションを当ててください」と次の手を示す。

| 案 | 採否 | 理由 |
|---|---|---|
| 専用のバリアントにする | **採用** | `Backend` は「こちらではどうにもならない」種類の失敗だが、これは**やることが決まっている** |
| `Backend` のまま | 却下 | 生のメッセージ（`relation "..." does not exist`）だけを見て原因を探すことになる |
| テーブルが無ければ検査を飛ばす | 却下 | スキーマが古いことを隠す。**気づかないまま古い帳簿を使い続ける**方が悪い |

### 実際に踏んだ

`fixed_assets`（0012）を追加した際、**本番の帳簿にマイグレーションを当て忘れた**。
その結果 `kaikei verify` が実帳簿で失敗し、出たのは次のメッセージだけだった。

```
財務諸表を組み立てられませんでした: 未分類のデータベースエラーです
（SQLSTATE 42P01）: relation "fixed_assets" does not exist
```

**何をすればよいかが書かれていない。** 今はこう出る。

```
帳簿のスキーマが古い可能性があります: …
コードが要求するテーブルが帳簿にありません。マイグレーションを当ててください
（MIGRATOR_DATABASE_URL を対象の帳簿へ向けて cargo run -p kaikei-store --bin kaikei-migrate）
```

### 外に出す文言では生のメッセージを伏せる

`public_message()` はテーブル名を含む DB のメッセージを出さない
（`docs/07-mcp-server.md` §9）。ただし**次の手は出す**——伏せた結果
「どうにもならない」としか読めない文言になると、分類を分けた意味が無い。
詳細は `Display` 側に残るので、サーバのログには `Display` を出す。

### 手順書にも入れた

`/kaikei-close` と `/kaikei-year-end` の前提に「帳簿のスキーマが最新か
確かめる」を追加した。エラーが分かりやすくなっても、**先に気づける方がよい**。


---

## D-111 家事按分は「記帳時」と「決算時一括」の両方を持つ

`household_split` は記帳時按分（3行）だけを提供しており、モジュールの doc に
「決算時一括按分は実装しない」と書いてあった。`year_end_household_split`
（2行）を足して、両方を選べるようにした。

### 実際の帳簿が決算時一括を必要とした

WeBanana.SP の2026年の帳簿は、自宅の家賃・電気代・携帯代を**全額そのまま
経費に計上**していた。7月分の家賃の仕訳には、摘要に本人の手で
「家賃7月分（セブンATM出金より充当・按分は確定申告時）」と書いてある。
**決算でまとめて按分するつもりで記帳していた**。

記帳時按分しか無いと、この帳簿は処理できない。既に計上した8か月分を
按分するには、後から家事分を経費から抜く形が要る。

### どちらが妥当かは決めない

`docs/08-compliance.md` §9-3 は「記帳時か決算時か」を税理士確認事項として
未解決のままにしている。**未解決だから片方しか作らない、という判断が
間違いだった。** どちらを選ぶかは利用者の判断であり、選べないこと自体が
実務の妨げになる。両方を提供し、どちらが妥当かは引き続き判断しない。

### 端数の丸め方を揃える

両方とも**事業分を先に計算し、家事分は引き算で求める**（D-063 と同じ）。
同じ総額・同じ割合なら、どちらの方法でも家事分が1円まで一致する。
揃えていないと、途中で方法を変えた年に差が出る。テストで
7つの金額 × 4つの割合 × 3つの丸め方を突き合わせて固定した。

### 税区分は逆の行に付ける

| | 税区分を付ける行 |
|---|---|
| 記帳時按分 | **事業分**の行（家事分は仕入税額控除の対象外） |
| 決算時按分 | **経費を減らす**行 |

決算時は、元の仕訳が全額に税区分を付けて計上している。控除しすぎた分を
同じ税区分で戻さないと、消費税の計算が合わない。

### CLI は出すだけ（D-103 / D-109 と同じ立場）

`kaikei household --year <西暦> --account <科目> --ratio <割合>` は金額を
出すだけで、記帳しない。**事業割合が妥当かはソフトには分からない**——
面積や使用時間の記録を持っていないからである。

対象科目も判定しない。自宅兼事務所の家賃なのか事業専用の事務所なのかを
知らないので、`--account` は人が指定する。

### `verify` では拾えない

減価償却の忘れは `verify` が指摘できる（固定資産台帳という「あるべき姿」が
帳簿の外にあるため）。**家事按分にはそれが無い。** 按分していない帳簿と、
事業専用で按分の要らない帳簿は、データとして見分けがつかない。

だから `/kaikei-year-end` の手順に入れた。**人が年に一度打つしかない。**

### 忘れると所得が過少になる

減価償却の忘れ（所得が過大）と向きが逆で、こちらは税務上不利になる。
2026年は8月13日時点で 2,029,972円 が按分前のまま計上されていた。

---

## D-112 家事按分は科目の一部だけも指定できる（`--amount`）

`kaikei household` は科目の年間計上額の全額を按分していた。`--amount` で
按分する額を指定できるようにした。

### 科目まるごとが対象とは限らない

実帳簿の通信費 476,631円 のうち、按分対象は携帯の 105,991円 だけである。
残り 370,640円 はドメイン・レンタルサーバー・AI サブスクリプションで
**事業専用**であり、按分すると経費を不当に減らすことになる。

地代家賃も同じ形になりうる。3月の不動産初期費用 515,720円（敷金・礼金・
仲介手数料等）は家賃と同じ割合で按分してよいか税理士確認待ちで、
別扱いになれば `--amount 1,330,000` で家賃分だけを按分する。

### 科目を分ければよい、とはしない

「携帯代」を独立した科目にすれば `--amount` は要らない。しかし**帳簿は
追記型で、過去の仕訳の科目は変えられない**。既に計上した分を按分するために
科目を作り直すことはできない。

### 計上額を超える額は受け付けない

帳簿に無い額を按分すると、経費がマイナスになる仕訳ができる。**貸借は
一致するので決算書を見ても気づけない**（D-101 と同じ形の危険）。
終了コード1で止める。

0以下も受け付けない。`year_end_household_split` 側でも弾かれるが、
CLI の引数として明確に誤りなので、計算に入る前に言う。

### 両方の額を必ず出す

```
  計上額 476,631 円
  うち按分対象 105,991 円（--amount で指定）
  残り 370,640 円は事業専用として按分しません
```

一部だけを按分したとき、どちらの額なのかが分からないと後から検算できない。

### 計上額は差引後（借方合計ではない）

通信費の借方合計は 537,462円 だが、返金5件（60,831円）が貸方に立っており、
**決算書に載るのは差引後の 476,631円** である。`--amount` の上限も
差引後の額を使う。SQL で借方だけを数えると、上限を超える額を指定して
しまいかねない。

---

## D-113 取得価額と償却方法の食い違いを指摘する（止めはしない）

`fixedasset add` は取得価額をまったく見ていなかった。50万円の資産に少額特例
（`immediate`）を当てても黙って通り、初年度に50万円が経費になる。

### なぜ気づけないのか

**貸借は一致したまま所得だけが減る。** `verify` でも拾えない——台帳に
書いてある方法で計算した結果と帳簿は一致してしまうからである。
D-101（決算振替を外さないと売上0になる）と同じ形の、決算書を見ても
分からない誤りである。
...
| 帯 | 扱い | 根拠 |
|---|---|---|
| 10万円未満 | 償却せず全額を必要経費 | 所令138条 |
| 10万円以上20万円未満 | 一括償却資産 | 所令139条 |
| 10万円以上30万円未満 | 少額減価償却資産の特例 | 措法28条の2 |
| 制限なし | 定額法 | — |
...
判定額は経理方式で変わり（タックスアンサー No.2100「取得価額の判定に際し、
消費税の額を含めるかどうかは納税者の経理方式によります」）、少額特例には
青色申告・年間300万円・貸付用でないことなど取得価額以外の要件もある。
**ここで見るのは金額の帯だけ**なので、断定できない（D-103 と同じ立場）。

`fixedasset add` は既定が下見なので、記帳の前に必ず目に入る。

### 令和8年度改正は取り込まない

少額特例は令和8年4月1日以後の取得について40万円未満に引き上げられたとされる
が、**税制改正大綱ベースの二次情報でしか確認できていない**。公布後の条文を
当たれていないので、**引き上げ後の額でこちらから通すことはしない**。
30万円で指摘を出し、引き上げの可能性を文面に添える。

### 税込経理で帯をまたぐ額を伝える

実帳簿の電動アシスト自転車等 108,000円（税込）は10万円以上だが、税抜なら
98,181円 で10万円を下回り、**全額を必要経費にできる区分**に落ちる。
経理方式の選択が償却額を左右するので、境目の近くではそれを言う。
境目から離れていれば言わない（毎回出る注意書きにしない）。

### 経理方式が無くても帯の検査は動かす

**設定が無いことを理由に、本体の検査まで止めない。** 帯の検査（所令138/139・
措法28の2）に経理方式は要らない。`KAIKEI_TAX_MODE` が読めなければ `None` を
渡し、帯をまたぐ額のときだけ「経理方式が分からないので確かめられませんでした」
と言う。値が壊れている場合も `None` にする——ここで止めると、設定を直すまで
資産を台帳に入れられなくなる。それはこの検査が背負う責任ではない。

最初は `jp_settings()?` を呼んでいて、**環境変数が無い環境で
`fixedasset add` そのものが落ちた**（E2E が2件落ちて気づいた）。
検査を足したせいで、検査と関係のない操作ができなくなっていた。

### `jp_settings` の決め打ちが誤りだった

家事按分（D-111）で `jp_settings` を書いたとき、按分の金額は丸め方だけで
決まるからと `tax_mode` に `Exclusive` を置いていた。この検査に使ったところ、
**税込経理の帳簿を税抜として扱った**。まさに気づきたかった場面で黙る形に
なっていたので、`KAIKEI_TAX_MODE` から読むようにした。

「この計算には影響しないから」で埋めた値は、**次に誰かが別の計算に使う**。

---

## D-114 取引先が無い課税仕入れを、少額特例の境目で分けて数える

`verify` は「適格請求書が要る税区分の明細が 603 件ありますが、取引先が
記録されていません」と件数だけを出していた。

### 件数だけでは動けない

603件と言われても手の付けようがない。少額特例（税込1万円未満は適格請求書の
保存が不要。28年改正法附則53の2）で分けたところ、実帳簿は
**570件 / 33件** になった。1万円以上は 33件・2,669,535円 で、いずれも
継続的な取引先である。**手を付けられる大きさかどうかが変わる。**

### 取引単位で数える（明細ごとではない）

1万円未満かどうかは「一回の取引の税込金額」で見る（国税庁「少額特例に
おける1万円未満の判定単位」）。**明細で分けると数が変わる**——1件の取引を
複数の明細に分けている帳簿では、どれも1万円未満に見えてしまう。

取引の額は借方の合計を使う。**貸方でも同じ数になる**（貸借は一致している）。
借方にするのは費用の側から見た「取引額」として読みやすいというだけで、
計算上の意味は無い。借方を貸方に変える変異はどのテストでも落ちないが、
それは当然である。

### 「対象になりうる」までしか言わない

少額特例には**事業者の規模**の要件がある——基準期間（個人は前々年）の課税
売上高1億円以下、または特定期間（前年1〜6月）5千万円以下。**このソフトは
判定しない。** 前々年の帳簿がこのシステムに無いことが普通にあるからである
（実際 weBanana.SP の帳簿は2026年から始まっている）。

### 何が免除されるのかを必ず添える

免除されるのは**適格請求書の保存だけ**で、帳簿の保存とその記載事項
（消法30条8項）は免除されない。「請求書が無くてもよい」と「帳簿に何も
書かなくてよい」は別のことである。E2E でこの一文が出ることを固定した。

### 期限も出す

令和11年9月30日で終わる（延長・恒久化されていない）。この日以後の課税
仕入れは対象外になる。**期限のある特例を、期限を言わずに勧めない。**

---

## D-115 適格請求書を揃えるべき取引を一覧にする

`verify` は「1万円以上は 32 件」と数えるが、**どの取引なのかを言わない**。
件数だけでは請求書を探しに行けない。`report` が
`invoices_to_collect.csv` を書くようにした（日付・仕訳番号・金額・摘要・
科目・科目名）。摘要があれば通帳やメールから元の取引を辿れる。

金額の大きい順に並べる。**先頭しか読まれないことがあるので、並び順が
「何を見せるか」になる**（D-098 と同じ理由）。

### 1万円未満は載せない

566件を並べても作業リストにならない。少額特例が使えるなら適格請求書の保存は
要らず、使えないとしても**まず1万円以上から**である。件数は `verify` が
伝えるので、黙って落としたことにはならない。

### 貸方に立つ課税仕入れは数えない・載せない

貸方の課税仕入れは「仕入れ」ではなく**返還**（返金・値引き）で、要るのは
適格請求書ではなく**適格返還請求書**である。同じ一覧に混ぜると
「請求書を探しても見つからない」ことになる。

実帳簿では 603件 のうち5件がこれだった（ドメイン代の返金 60,831円）。
一覧に出して初めて気づいた——**件数だけを見ていたときは分からなかった。**

返還の側の検査は作っていない。必要になったら別に作ること。

### 数える側と並べる側で条件を二重に書かない

`split_by_small_amount`（数える）と `invoices_to_collect`（並べる）が
違う条件を持つと、件数と一覧の行数が食い違ったときにどちらが正しいのか
分からなくなる。実際、貸方を外したとき最初は一覧だけ直して
**32件 と 33件 に割れた。** 判定を `needs_an_invoice` に寄せ、借方の
絞り込みも両方に入れた。

### 分母を必ず言う

上の件数（603・**明細**）と内訳（598・**取引**・借方のみ）は数え方が違う。
分母を出さないと「足しても合わない」と読まれる。

```
  このうち借方に立つ 598 取引を、税込1万円で分けると:
    1万円未満 566 件（少額特例の対象になりうる）
    1万円以上 32 件・2,625,568 円（適格請求書の保存が要ります）
  ※ 貸方に立つ課税仕入れ（返金・値引き）はここに数えていません。
```

### 内訳は1万円未満が0件でも出す

最初 `split.small > 0` で囲っていたため、**全部が1万円以上のときに内訳ごと
消えていた**（E2E を書いて気づいた）。「0件」は言うに値する情報である。

---

## D-116 取引先が無い課税仕入れは、借方と貸方を分けて数える

`verify` は貸借を区別せず数えていた。**家事按分を記帳したら3件増えた。**

### 内部の振替に相手方は存在しない

家事按分の振替仕訳は、貸方に課税仕入れの科目（地代家賃・水道光熱費・通信費）
が立つ。既存の数え方はこれを「取引先が記録されていない課税仕入れ」として
拾い、**「請求書を揃えてください」と言っていた。** 相手方が存在しないので
的外れである。
...
| 何か | 要る書類 |
|---|---|
| 返金・値引き（仕入対価の返還） | **適格返還請求書** |
| 家事按分などの内部の振替 | 無い（相手方が存在しない） |

どちらも「適格請求書を揃える」ではない。**自動では区別できない**ので、
両方を挙げて理由を書く。

### 見出しの数と内訳の算数を合わせる

見出しが 606（明細・貸借込み）で内訳が 598（取引・借方のみ）だと、
読み手が引き算をして合わないことに気づく。見出しで内訳を出す。

```
注意: 課税仕入れの明細 606 件に取引先が記録されていません。
  うち 8 件は貸方に立っています（返金・値引き、または家事按分などの内部の振替）。
  借方の 598 取引を、税込1万円で分けると:
```
...
毎回出る注意書きにしない。E2E で「貸方が無いときは貸方の話をしない」ことを
固定した。

---

## D-117 証憑は仕訳番号でも紐付けられる（`--entry-no`）

`kaikei attach` は `--entry <UUID>` と `--match-amount <円>` しか受けなかった。

### 一覧が出すのは仕訳番号である

`invoices_to_collect.csv`（D-115）は仕訳番号を出す。UUID しか受けないと、
**一覧の行ごとに帳簿を引き直して UUID を調べる**ことになる。実帳簿では
32件がその対象で、それをやらせる設計に意味が無い。

### 金額で引くのとの違い

仕訳番号は**年度の中で一意**なので候補が複数になることがない。
`--match-amount` は同じ額の仕訳が複数あると止まる（毎月同額のサブスク等）。
一覧から作業するなら番号の方が確実である。

### 年が決まらなければ止める

仕訳番号は年度の中でしか一意でない。`--match-year` も `--date` も無ければ
**どの年の何番か決まらない**ので止める。`--match-amount` と同じ規則。

### 3つを同時には受けない

`--entry` / `--entry-no` / `--match-amount` のどれを使ったのか分からないまま
登録されると、意図しない仕訳に紐付いても気づけない。**紐付けは追記のみで
消せない。**

### 無い番号は黙って通さない

年と番号を出して止める。紐付けずに登録すると「付けたつもりで付いていない
証憑」ができ、証憑の一覧を見ても気づけない。

---

## D-118 済んだ分は作業リストから落とす

`invoices_to_collect.csv`（D-115）は、証憑を紐付けても行が残り続けていた。

### 減らない一覧は作業リストとして使えない

32件を順に片付けても件数が変わらなければ、**どこまで進んだか分からない**。
請求書を集める作業は数日〜数週間にまたがるので、途中経過が見えないと
「もう一度最初から確かめる」ことになる。

請求書（`invoice`）か領収書（`receipt`）が紐付いている仕訳は一覧から外し、
**外した件数を出す**（黙って減らすと「なぜ減ったのか」が分からない）。

### 契約書では落とさない

契約書（`contract`）が付いていても、**その取引の請求書が揃ったことには
ならない**。種別を見る。

### 取引先までは求めない

証憑に取引先が入っていなくても、**請求書そのものは揃っている**。
取引先が空なら `attach` が別に警告する（検索要件の話であって、「集める」
作業とは段が違う）。

### 一覧に残っている分だけ引く

`documents_of_entry` は仕訳1件ごとの問い合わせである。600件全部に投げると
重いので、**一覧に残っている数十件だけ**を見る。

### `verify` の件数は減らない

これは意図どおりである。`verify` が数えているのは「**帳簿に**取引先が記録
されていない明細」で、証憑を付けても帳簿は変わらない（追記型なので変えられ
ない）。**証憑があれば帳簿の記載事項を満たすのか**は税理士確認事項として
未解決（`docs/09-tax-research.md`）なので、こちらが勝手に「解決した」ことに
はしない。

進捗が見えるのは作業リストの側だけでよい。

---

## D-119 弥生の未確認の写像は「使っている区分」だけを挙げる

`report --yayoi` は「未確認 8 件」とだけ出していた。

### 数字だけでは動けない

8件のうちどれが自分の帳簿に効くのか分からない。表に載っているだけで一度も
使っていない区分まで数えているからである。実帳簿（2026年）が使うのは
**4区分**で、残り4つは確かめても意味が無い。

確認を求める相手は**税理士**である。**確かめる対象を絞れないと、依頼そのものが
重くなる。**

```
注意: 弥生の税区分の写像を実機で確認していません。この帳簿が使っている 4 件が未確認です:
  OUT_OF_SCOPE → 対象外
  PURCHASE_10_QUALIFIED → 課対仕入込10%適格
  SALES_10 → 課税売上込10%
  TAX_FREE → 非課売上 / 非課仕入
  取り込む前に税理士に確認してください。
  （表は全 8 件が未確認ですが、残りはこの帳簿で使っていません）
```

**弥生側の名前も出す。** コード名だけでは、何を確かめてもらうのか伝わらない。

### 売上と仕入で名前が変わる区分は両方出す

`TAX_FREE` は売上なら「非課売上」、仕入なら「非課仕入」になる。片方だけ
確かめてもらっても、もう片方が誤っていれば取り込みで壊れる。

### 全部が確認済みなら文面を変える

使っている区分がすべて確認済みなら「未確認のものが N 件ありますが、この
帳簿では使っていません」と言う。**黙るのではない**——写像が未確認である
事実は変わらないので、伝えたうえで自分には効かないと言う。

### 同梱の表では検査できない分岐がある

`verified` を無視する変異を入れても、**同梱の表を使うテストは1件も落ちなかった**
——表が全件 `verified: false` だからである。確認が進むほど効いてくる分岐なので、
手で組んだ表（一部を `verified: true`）で固定した。

**「テストが通った」は「その分岐が守られている」ではない。**

---

## D-120 毎月同額の支出が違う科目に入っていたら知らせる

`verify` に `inconsistent_account` を足した。
...
**所得は変わらない**（どちらも経費）。変わるのは決算書のどの欄に載るかで、
同じサブスクリプションが月によって違う欄に載る状態は帳簿の質の問題である。

### 条件を絞らないと誤検出する

「同じ額で科目が違う」だけでは足りない。実帳簿で試したところ、1,000円 の
支出が旅費交通費・新聞図書費・会議費の3科目に分かれて拾われた——**別々の
取引が偶然同じ額だっただけ**である。

| 条件 | なぜ |
|---|---|
| 3回以上 | 2回では偶然と区別できない |
| 3か月以上に散る | 同じ月に3回は「毎月の支出」ではない |
| 日にちの種類が2つまで | 定期的な支出は日付が揃う。引落日は土日でずれるので1つに固定しない |

**この3つを足したら、誤検出が消えて狙った2件だけが残った。**

### 費用の科目だけを見る

絞る前は 2,280円 の指摘に **325 未払金**（カードの引落し）が混ざっていた。
同じ額の入出金が違う科目に入るのは普通である（振替・立替・カードの引落し）。

### 断定しない

「同じ取引なら科目を揃えてください。別々の取引が偶然同じ額なら、そのままで
構いません」と書く。**どちらかはソフトには分からない。**

---

## D-121 同じ費用科目で税区分が割れていたら知らせる

`verify` に `inconsistent_tax_category` を足した。

### 科目の割れ（D-120）より重い

科目の割れは**所得を動かさない**が、税区分の割れは**仕入税額控除の額を
変える**。

### 実際に見つけたもの

実帳簿（2026年）の地代家賃6件のうち、**5件が課税仕入10%（適格）、1件が
非課税**だった。課税として扱っている額は 1,640,720円 で、控除額にすると
約 149,156円 になる。

住宅の貸付けは非課税（消費税法別表第二第十三号）なので、自宅の家賃を課税
仕入として控除していれば控除のとりすぎになる。逆に事務所であれば課税が
正しく、非課税の1件が誤りになる。

### 少数派を「誤り」と呼ばない

**多数決で決めない。** 上の例では5件と1件のどちらが誤りかが決まらない
（契約の中身をソフトは知らない）。件数を並べて、判断は人間に返す。

### 閾値を足していない

「少数派が全体の N% 未満なら」のような絞りを入れたくなるが、**実帳簿で誤検出が
出ていない**（割れている科目は1つだけ）ので、確かめようのない条件を先回りで
足さない。旅費交通費のように課税と対象外が正当に混ざる科目が出てきたら、
そのとき実物を見て決める。

D-120 では日付・月数の絞りを足したが、あれは**実際に誤検出が出たから**である
（1,000円 の支出が3科目に分かれて拾われた）。**出ていない誤検出に対策しない。**

### テストで踏んだこと

`TagSchema::empty()` はタグを一切拒む。税区分タグが付いた仕訳を作る時点で
落ちるので、テスト用に `tax_category` を受け付けるスキーマを用意した。

---

## D-122 取引先が付いていても、登録番号が未確認なら知らせる

`verify` は取引先タグの**有無**しか見ていなかった。**タグが付いていても、
その相手が適格請求書発行事業者かどうかを確かめていなければ、仕入税額控除の
根拠にならない。**

### 実帳簿の状態

取引先マスタ31件が**すべて未確認**（`invoice_registration_no` も
`is_qualified_invoice_issuer` も空）。freee 側の取引先34件も登録番号が
全件未入力である。

取引先タグを付けた5件（Apple / Google / Microsoft / povo / 東急パワーサプライ）
も、この意味では未確認のままだった。**タグを付けた達成感で止まる形になって
いた。**
...
|---|---|
| 適格と確認済み | 全額控除 |
| **非適格と確認済み** | **経過措置で処理できる**（80%／2026年10月以降70%） |
| 未確認 | どちらの扱いもできない |

`is_qualified_invoice_issuer` が `false` は「非適格だと確認した」であって、
`None`（未調査）とは別である。**この2つを混ぜない**（`list_partners` と
同じ立場）。だから**未確認だけを数える。**

### 判定を明細1行の関数に切り出した

`unverified_counterparty_name` として分けた。`line_needs_a_counterparty` と
同じ理由——**何を拾うかをテストで固定できないと、呼び出しごと消えても
気づけない。** CLI のテストは仕訳ではなくタグ集合を組み立てるので、
そこに合わせた。

### 取引先マスタに無いコードは拾わない

ここは整合性を検査する場所ではない。マスタに無いコードが仕訳に入っている
こと自体は別の問題で、記帳時に `TaxPolicy::validate_tag` が見ている。

---

## D-123 売上があるのに債権・債務が動いていなければ知らせる

実帳簿は**入出金ベース**で記帳されていた。売上11件すべてが「普通預金／
売上高」で、売掛金の明細は**0件**。買掛金・前受金・前払金も0件だった。

### 決算で足りない

個人事業主の原則は**発生主義**（所得税法36条1項「その年において**収入すべき**
金額」）。入金した日に売上を立てる形のままだと、**12月に役務提供が終わって
1月に入金される分がその年の所得に入らない。**

現金主義の特例（所法67条）は前々年の事業所得等が300万円以下という要件があり、
この帳簿（2026年の所得は約970万円）では使えない。
...
売掛金が0件でも、**その年のうちに全部入金されていれば正しい**。だから
「入出金ベースになっていないか確かめてください」と書き、「0のままで正しい
場合もあります」を添える。
...
科目コードではなく名前（売掛金・買掛金・前受金・前払金）で見ている。
コードの体系は帳簿ごとに違いうるが、この4つは青色申告決算書の様式にある。

**名前が変わると黙って効かなくなる**ので、同梱の勘定科目表にこの名前が実在
することをテストで固定した（`the_chart_still_has_the_names_this_check_needs`）。
実際、名前を1つ変える変異を入れると2件落ちる。

---

## D-124 種別を足したら分類も直す（`is_suspicion` を網羅的な `match` に）

D-120（`InconsistentAccount`）と D-121（`InconsistentTaxCategory`）を足した
とき、`FindingKind::is_suspicion` を更新し忘れた。その結果、**実帳簿の
`kaikei verify` が終了コード1で失敗するようになっていた。**

どちらの doc にも「**誤りとは限らない**」と書いておきながらである。
...
```rust
matches!(self, FindingKind::SuspectedDuplicate)  // 新しい種別は false になる
```

そして**終了コードを確かめていなかった**。`verify | grep ...; echo $?` は
grep の終了コードを見ているので、0 が出て通っているように見えた。

### なぜ露見しにくいか

**この形の抜けは、正しい帳簿でしか露見しない。** 不整合が別にある帳簿で試すと
どちらにせよ失敗するので気づけない。実帳簿は不整合0件だったからこそ、
「疑いだけで失敗する」が表に出た。

### 直し方

網羅的な `match` にした。**種別を足すとコンパイルが通らない**（変異を入れて
確かめたところ3件のエラーが出る）。

あわせて、すべての種別がテストでどちらかに分類されていることを件数で
固定した。**種別を足してテストに書き忘れたら落ちる。**

---

## D-125 「疑いだけなら失敗しない」を実バイナリで固定する

D-124 の回帰（新しい種別を `is_suspicion` に足し忘れて `verify` が終了コード1に
なった）は、**単体テストを足しただけでは再発を防げない**。分類の関数が正しくても、
CLI が終了コードをどう決めるかは別の場所にあるからである。

`kaikei-e2e/tests/cli_verify.rs` に**実バイナリを動かすテスト**を2件足した。

| テスト | 見るもの |
|---|---|
| `advisory_findings_alone_do_not_fail_the_check` | 疑いだけの帳簿で**終了コード0** |
| `a_real_inconsistency_fails_the_check` | 本物の不整合で**終了コード1** |

後者が無いと、前者は「常に成功する」実装でも通ってしまう。

### 正しい帳簿でしか露見しない

**疑いだけがある帳簿**を作る必要がある。不整合が別にある帳簿で試すと、
どちらにせよ失敗するので気づけない。だから「毎月ほぼ同じ日に同額の支出が
違う科目に入っている」形（`inconsistent_account`）を作った——帳簿としては
何も壊れていない。
...
| 作ろうとしたもの | 拒まれた理由 |
|---|---|
| 仕訳番号の重複 | `journal_entries_fiscal_year_entry_no_key` |
| 存在しない仕訳を指す赤伝 | `reverses` は自己参照の外部キー |
| 理由の無い赤伝 | `CHECK ((reverses IS NULL) = (reverse_reason IS NULL))` |

**前年の仕訳を訂正した赤伝**で作れた。`dangling_reversal` は検査する年度の中に
訂正元が居るかを見るので、前年を指していれば「見当たらない」になる。
...
## D-126 「決算振替を記帳しても決算書が変わらない」を実バイナリで固定する

D-101（決算書は決算振替を外して集計する）は `usecase::statements` の層では
テストがあるが、**出力されたファイルが前後で同じであることは誰も見ていなかった**。

手順書（`/kaikei-year-end` の 4）は「前後で diff を取れ」と書いているが、
**人がやるので忘れる。**

外し方を壊すと**決算書が静かに空になる**——売上0・所得0の決算書ができ、
貸借は一致したままなので見ても気づけない。

### 対になるテストを置く

| テスト | 見るもの |
|---|---|
| `the_blue_return_does_not_change_when_the_closing_entry_is_posted` | 決算書が**バイト単位で同じ** |
| `the_journal_book_does_grow_when_the_closing_entry_is_posted` | 仕訳日記帳は**増える** |

後者が無いと、「どの出力も変わらない」実装でも前者は通る。

また前者では「そもそも決算書に金額が載っている」ことも見る。
**両方とも空なら「同じ」は成り立つ。**

### 変異が空振りした

最初、`exclude_closing_on: Some(year.end())` を `None` に変えて試したが
**テストが緑のままだった**。数えると3箇所ともに置換されているのに、である。

原因は場所を間違えていたこと。`write_reports`（決算書を出す側）は
`Some(to)` を使っており、変えた3箇所は `verify` と `household` の側だった。

**「変異を入れたのにテストが通った」を「テストが弱い」と読む前に、変異が
効いているかを確かめること。** ここでは同じ意味の式が2通りの書き方で
散らばっていたのが原因である。

---

## D-127 期首振替を年内に記帳したら知らせる

D-102 は期首振替を**翌年1月1日**に記帳すると決めた。年内（12月31日など）に
入れると、**決算書の貸借対照表から事業主貸・事業主借が消える。**

青色申告決算書の様式にはこの2欄があり、期末残高をそのまま書く。0 で提出する
ことになる。

### 実際に試したら再現した

実帳簿の複製で12月31日に期首振替を入れたところ、**事業主貸 9,923,381円 と
事業主借 1,012,434円 が決算書から消え、`verify` は終了コード0のまま**だった。

手順書は「翌年1月1日に記帳する」と書き、忘れた場合の検査
（`warn_if_owner_accounts_carried_over`）もあったが、**早すぎた場合を見る
ものが無かった。**

### 期首振替そのものも「動き」に数える

最初は「期首振替が原因で0になっているので、動きから除く」と考えて外していた。
**それだと見逃す場面がある。**

期首振替は**前年からの繰越**を振り替えるので、その年に事業主貸を1度も使って
いなくても振替は起きる。除くと「何も動いていない」ことになり、黙って通る。

正しく翌年1月1日に記帳された期首振替は、そもそもこの年度の仕訳に入ってこない
（日付で外れる）。**この年度に居る期首振替は、それ自体が誤りである。**

### 変異テストが偽陰性を見つけた

除く分岐を消す変異を入れたところ、**どのテストも落ちなかった**。テストが弱いの
かと思って場面を作ったら、**実装のほうが間違っていた**（前年繰越があって当年に
動きが無い帳簿を見逃す）。

最初に作ったテストは前年の仕訳を入れておらず、そもそも残高が0にならなかった。
**「テストが落ちない」の原因は、テストの作りが実態と違うことも、実装が
間違っていることもある。**

---

## D-128 消費税の申告に向けた集計を出す（申告書は作らない）

利用者は**課税事業者・原則課税**である。確定申告には所得税だけでなく消費税の
申告も含まれるが、kaikei には集計が無かった（`docs/04-jp-tax.md` は Phase 4
以降扱いにしていた）。

税区分は全明細に付いているので、集計はできる。

### 申告書は作らない

出すのは集計値だけである（`depreciation` / `household` と同じ立場）。
**反映していないものを必ず並べる。**

| 反映していないもの | なぜ |
|---|---|
| 家事按分 | どの科目が按分対象かは帳簿から決まらない |
| 適格請求書発行事業者かどうかの確認 | 取引先の登録番号を確かめていない（D-122） |
| 経過措置の控除割合（80%／70%） | 非適格の区分が使われていれば別に集計する |
| 端数処理の規定 | 申告書上の規定であり、帳簿の集計とは別 |
| 課税売上割合による按分 | 非課税売上がある場合 |

### 割り算を最後にする

税込経理なので税額は割り戻す。**`rate / (1 + rate)` を先に求めると桁が落ちる**
——10% で 0.0909090909… という割り切れない小数になり、12,070,080円 の税額が
1,097,279円 になった（正しくは 1,097,280円）。

掛けてから割れば 1,207,008 ÷ 1.1 = 1,097,280 で割り切れる。

### 貸方を差し引く

返金・値引き（対価の返還）は課税仕入れ・課税売上を減らす。借方だけを足すと
控除額が過大になる。実帳簿では返金5件・60,831円 がこれに当たる。

### 集計できなかった明細を数えて言う

税区分の無い明細と、表に無いコードの明細を数える。**黙って捨てると、集計が
合っていないことに気づけない。** 実帳簿では税区分の無い明細が774件あるが、
その大半は口座や事業主貸で、税区分を持たないのが正しい。**だから断定せず、
件数と理由を添える。**

### 前提が違えば止める

税抜経理・簡易課税では**黙って誤った数字を出すより止める**。税抜経理で割り
戻すと税額を二重に数える。簡易課税は事業区分ごとのみなし仕入率が要る。

### 桁揃えを当てにしない

`{:<28}` は char 数で数えるので、全角混じりでは揃わない。**揃えようとして
崩れるより、区切り記号で読ませる。**

---

## D-129 CI は PR ブランチへの push で走らせない

`architecture.yml` / `database.yml` / `supply-chain.yml` が `on: push` と
`on: pull_request` の両方に反応しており、**同じコミットに対して同じジョブが
2回走っていた。**

### 実害が出た

PR #117 で `database` の片方が **27分キューに滞留**し、必須チェックが揃わず
マージできなくなった。もう片方は同じコミットで4分28秒で通っている。

空コミットで再実行しても同じことが起きた。**重複そのものが、詰まる確率を
倍にしていた。**

### 直し方

`push` を `branches: [main]` に絞る。PR ブランチでは `pull_request` 側だけが
走り、main へのマージ後にもう一度走る。

**必須チェックの名前は変わらない**ので、ブランチ保護の設定はそのままでよい。

### `commit-hygiene.yml` は元から `pull_request` だけ

そちらは触っていない。`supply-chain.yml` の `schedule`（毎週の脆弱性検知）も
残してある。

---

## D-130 消費税の集計を `report` の出力に入れる

D-128 で `kaikei consumptiontax` を作ったが、**税理士へ渡す一式には入って
いなかった。**

**画面で読む人と、ファイルを受け取る人は別である。** 確定申告には消費税の
申告も含まれるので、決算書と一緒に出す。

### 注記も一緒に書き出す

`consumption_tax.csv`（表）と `consumption_tax_notes.txt`（反映していないもの）
を分ける。1つの CSV に表と注記を混ぜると表計算で読めなくなる
（`blue_return_not_on_form.csv` と同じ理由）。

**数字だけ渡すと申告書の金額と読まれる。** 注記が画面にしか無ければ届かない。

### 前提に合わない帳簿では出さない

税抜経理・免税事業者・簡易課税では書き出さない。**黙って誤った数字を渡すより、
無い方がよい。**

### 翻訳は1箇所に寄せる

仕訳 → `TaggedLine` の変換を `tagged_lines_for_consumption_tax` にまとめ、
`report` と `consumptiontax` の両方から呼ぶ。**2箇所に書くと、片方だけ直した
ときに画面とファイルで数字が食い違う。**

### `kaikei-report` が `kaikei-jp` に依存してよい理由

CI は `kaikei-report` が I/O 層（sqlx / tokio / kaikei-store / kaikei-import /
kaikei-mcp）に依存しないことを検査している。`kaikei-jp` は I/O を持たない
計算の層なので、この crate の性質（データ → 文字列）を壊さない。

### 変異が空振りした（3度目）

`written.push(notes_path)` を消しても**どのテストも落ちなかった**。
`written` は「書き出したファイルの一覧」を表示するためのもので、**ファイル
自体は書かれている**からである。中身を空にする変異に変えたら落ちた。

**何を消したかで、何が検査できるかが決まる。**

---

## D-131 CI のジョブとステップに時間の上限を置く

`database` ジョブが `postgresql-client をインストール` のステップで
**16分以上ハング**した。`apt-get update` がミラーの応答を待ち続けていた。

**GitHub Actions の既定は6時間である。** 放っておくと、必須チェックが揃わない
まま6時間待つことになる（気づかなければもっと長い）。

### 2段構えにする

| 置く場所 | 上限 | なぜ |
|---|---|---|
| `apt-get` のステップ | 5分 | 止まる場所が分かっているので短く切る |
| ジョブ全体 | 20〜25分 | 他のどこで止まっても6時間待たない |

`database` は通常4分半で終わるので25分あれば遅い日でも足りる。

### そもそも apt を触らない

上限を置いたら**5分で失敗するようになっただけ**だった。ミラーが落ちていれば
取得できないのだから当然である。

`psql` は**GitHub のランナー画像に最初から入っている**。それでも毎回
`apt-get update` を走らせていた。**要らない仕事が、一番弱い依存を毎回踏んで
いた。**

入っているかを先に見て、あれば apt を触らない。無いときだけ入れる。
...
ミラーが1つ落ちていても次へ進む。**ステップの `timeout-minutes` は「諦める」
だけで、「やり直す」ことはしない。**

### 前回の説明は誤りだった

D-129 で「重複起動が詰まりの原因」と書いたが、**重複を解消した後も同じ滞留が
起きた**。原因は `apt-get` のハングで、重複とは無関係だった。

重複解消自体は有効（チェック9件 → 5件、CI 時間が半分）だが、**因果を取り違えた
まま「直った」と言っていた。**

---

## D-132 `report` は税制の設定が無くても動く

D-130 で消費税の集計を `report` に足したとき、`jp_settings()?` を呼んだ。
**`KAIKEI_ROUNDING` / `KAIKEI_TAX_MODE` が無い環境で `report` 全体が落ちる
ようになった。**

手元では `.env` を読んだシェルから走らせていたので気づかず、**CI の E2E が
10件落ちて**分かった。

**D-113 とまったく同じ形である。** あのときは `fixedasset add` に同じことを
して、同じく E2E で気づいた。**2度目なので、テストの側に仕掛けを入れた。**

### `report` は帳簿を書き出すコマンドである

税制の設定が無くても動かなければならない。消費税の集計に丸め方は要らない
（税込から割り戻すだけ）ので、**経理方式だけを任意で読む**。読めなければ
消費税の集計を出さない——**黙って空の集計を出すよりよい。**

### テストの既定を「設定を渡さない」にした

`run_report` は税制の設定を1つも渡さない。渡してしまうと、「設定が要る処理」を
足したときに気づけない。消費税の集計を検査するテストだけが
`run_report_with(..., &[("KAIKEI_TAX_MODE", "inclusive")])` で明示的に渡す。

### 親の環境変数を子に引き継がせない

`Command` は既定で親の環境を引き継ぐ。手元では `.env` を読んだシェルから
走らせるので、**渡していないつもりの設定が子プロセスに届く**。それだと
「設定が無くても動く」を検査したつもりで検査できていない。
...
## D-134 取引先の適格性だけを後から記録できるようにする

`counterparty import` は `ON CONFLICT (code) DO NOTHING` なので、**既存の
取引先に登録番号を後から入れられなかった。** 実帳簿の31件はすべて空で、
CSV から入れ直そうとしても「既存を優先」で無視されていた。

**下見は「書ける」と表示するのに、実際は書き込まれない**（警告は出る）という
紛らわしい状態だった。

### 相手の適格性は後から分かる

取引を記帳した時点では未確認で、あとで先方に伺って埋める。**追加しかできない
と、その運用が成り立たない。**
...
| コマンド | 何をする | 既存行 |
|---|---|---|
| `counterparty import` | まとめて取り込む | **触らない** |
| `counterparty verify` | 調べた結果を記録する | **上書きする** |

同じコマンドにすると、取り込みのつもりが上書きになる。既存の
`insert_counterparties` の doc（「`DO UPDATE` に書き換えないこと」）はそのまま
残し、別の口を足した。

### 名前は変えない

更新するのは `invoice_reg_no` / `is_qualified` / `verified_at` の3つだけ。
**名前を変えられると、過去の仕訳が指している相手が静かに別物になる。**
SQL の `SET` に `name` を足す変異を入れたら3件落ちる。

### 省略した項目は既存の値を残す

`None` をそのまま渡すと消える。登録番号を入れた後に `--qualified` だけを
直したとき、番号が消えてはいけない。**変異を入れたら1件落ちる。**

### 何も変えない呼び出しを断る

`--registration-no` も `--qualified` も無ければエラーにする。打ち間違いで
落とすと、**確認日だけが更新されて「確認したのに何も分からなかった」**状態に
なる。

### 「未確認」を true/false で表さない

`--qualified` を付けなければ未確認のまま。`false` は「**非適格だと確認した**」
であり、経過措置で処理できる（D-122）。エラーメッセージでもその区別を言う。

### 書き込む前に形を確かめる

登録番号はチェックデジットまで検証する（`InvoiceRegistrationNo::parse`）。
