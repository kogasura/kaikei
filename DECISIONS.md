...
（`カ)サンプル シヨウジ 12345` の番号）。全体で探すと1件も当たらないので、
...
| 何も出さない | 却下 | **`JpTaxPolicy` は取引先タグが有るときにしか適格性を見ない。** 無いまま記帳すると検証がすり抜ける。検証帳簿では大量の仕訳がこの状態だった |
...
「非適格だと確認済み」に仕立てることになる。**

**入力内の重複はエラーにしない**: 外部システムには表記ゆれの重複が普通に
ある（外部の会計サービスには「株式会社 ABC」と「株式会社ABC」が別々に
...
**既存の 248 件には後から付けられない**: `journal_lines` は追記型で
...
| 案 | 採否 | 理由 |
|---|---|---|
| `entry_kind` タグで印を付け、決算書から外す | **採用** | 記帳の順番に関係なく決算書が同じになる。検証帳簿の複製で検証済み（下記） |
...
**検証**: 検証帳簿の複製（検証用DBの TEMPLATE）に決算振替を記帳し、
...
最初は両方 `closing` にしていた。検証帳簿の複製で2027年の決算書を出したところ、
期首振替を記帳済みなのに**事業主貸 8,052,438 が貸借対照表に残った**。
...
合わなかった**（資産 8,141,943 に対し負債・資本 905,600）。
`only_opening_on` で「その日は期首振替だけを入れる」とした。

**期首残高の仕訳は前年末日付で入れること。** 1月1日に立っている仕訳が期首の姿
なのか初日の商売なのかは日付だけでは区別できず、区別できるのは
`entry_kind: opening` の印だけである。検証帳簿もそうなっている
（「前期末日付で計上」）。

### 検証帳簿の複製での検証

検証用DBの複製に決算振替と期首振替の両方を記帳した。

| | 2026 | 2027 |
|---|---|---|
| 事業主貸 期末 | 8,052,438 | **0** |
| 事業主借 期末 | 820,512 | **0** |
| 元入金 | −1,437,985 | **−2,040,211** |
| 貸借対照表 合計（期首/期末） | 64,400 / 8,141,943 | 92,000 / 92,000 |

2026年の決算書は記帳の前後で1バイトも変わらない（D-101）。
2027年の元入金 −2,040,211 は手計算（−1,437,985 + 6,629,700 − 7,231,926）と一致する。
...
| 取得価額から償却方法を決める | 却下 | 20万円未満・30万円未満は**選べる**のであって決まるわけではない。実際 105,600円 の資産には3つの扱いがあり、初年度の償却費は 44,000／35,200／105,600 と大きく違う |
...
2022-08-05 取得の「pc」97,200円 は、検証帳簿で 2022〜2024年に 32,400円ずつ
...
検証帳簿の資産は割り切れる耐用年数で、たまたま影響が無かった。**割り切れる耐用年数しか
...
検証帳簿では3科目とも食い違っており、原因が科目ごとに違った。

### 検証帳簿で出た指摘

```
注意: 固定資産台帳の期末簿価と帳簿の残高が違う科目が 3 件あります:
  205  台帳 0 円 / 帳簿 97200 円（差 97200）
  210  台帳 152180 円 / 帳簿 140405 円（差 -11775）
  220  台帳 9000 円 / 帳簿 105600 円（差 96800）
```

| 科目 | 何が起きているか |
|---|---|
| 205 機械装置 | 2022年取得の pc は2024年で償却し終えているのに、**償却の相手科目を取り違えていた**ため取得価額のまま残っている |
| 220 車両運搬具 | 2025・2026年の償却が**未計上**（44,000 + 52,800 = 96,800） |
...
`fixed_assets`（0012）を追加した際、**本番の帳簿にマイグレーションを当て忘れた**（0012 はマイグレーション番号。ci-allow: real-ledger-mention）。
その結果 `kaikei verify` が検証帳簿で失敗し、出たのは次のメッセージだけだった。
...
検証帳簿の2026年は、自宅の家賃・電気代・携帯代を**全額そのまま
経費に計上**していた。家賃の仕訳の摘要には「按分は確定申告時」と
書かれており、**決算でまとめて按分するつもりで記帳されていた**。
...
2026年は8月13日時点で 1,690,000円 が按分前のまま計上されていた。
...
検証帳簿の通信費 386,880円 のうち、按分対象は携帯の 86,020円 だけである。
残り 300,860円 はドメイン・レンタルサーバー・AI サブスクリプションで
**事業専用**であり、按分すると経費を不当に減らすことになる。

地代家賃も同じ形になりうる。3月の不動産初期費用 418,900円（敷金・礼金・
仲介手数料等）は家賃と同じ割合で按分してよいか税理士確認待ちで、
別扱いになれば `--amount 1,080,000` で家賃分だけを按分する。
...
```
  計上額 386,880 円
  うち按分対象 86,020 円（--amount で指定）
  残り 300,860 円は事業専用として按分しません
```
...
通信費の借方合計は 436,260円 だが、返金の数件（49,380円）が貸方に立っており、
**決算書に載るのは差引後の 386,880円** である。`--amount` の上限も
...
検証帳簿の電動アシスト自転車等 105,600円（税込）は10万円以上だが、税抜なら
96,000円 で10万円を下回り、**全額を必要経費にできる区分**に落ちる。
...
`verify` は「適格請求書が要る税区分の明細が 248 件ありますが、取引先が
記録されていません」と件数だけを出していた。

### 件数だけでは動けない

248件と言われても手の付けようがない。少額特例（税込1万円未満は適格請求書の
保存が不要。28年改正法附則53の2）で分けたところ、検証帳簿は
**231件 / 17件** になった。1万円以上は 17件・2,167,000円 で、いずれも
...
`verify` は「1万円以上は 16 件」と数えるが、**どの取引なのかを言わない**。
...
229件を並べても作業リストにならない。少額特例が使えるなら適格請求書の保存は
...
検証帳簿でも数件がこれだった（ドメイン代の返金 49,380円）。
...
**16件 と 17件 に割れた。** 判定を `needs_an_invoice` に寄せ、借方の
絞り込みも両方に入れた。

### 分母を必ず言う

上の件数（248・**明細**）と内訳（245・**取引**・借方のみ）は数え方が違う。
分母を出さないと「足しても合わない」と読まれる。

```
  このうち借方に立つ 245 取引を、税込1万円で分けると:
    1万円未満 229 件（少額特例の対象になりうる）
    1万円以上 16 件・2,131,000 円（適格請求書の保存が要ります）
...
見出しが 251（明細・貸借込み）で内訳が 245（取引・借方のみ）だと、
読み手が引き算をして合わないことに気づく。見出しで内訳を出す。

```
注意: 課税仕入れの明細 251 件に取引先が記録されていません。
  うち 6 件は貸方に立っています（返金・値引き、または家事按分などの内部の振替）。
  借方の 245 取引を、税込1万円で分けると:
```
...
**一覧の行ごとに帳簿を引き直して UUID を調べる**ことになる。検証帳簿では
16件がその対象で、それをやらせる設計に意味が無い。
...
使っていない区分まで数えているからである。検証帳簿（2026年）が使うのは
...
| 取引 | 額 | 割れ方 |
|---|---:|---|
| 動画サブスクリプション（毎月2日） | 2,280 | 1・2・3・5月は通信費、4・6・7・8月は新聞図書費 |
| 記事サブスクリプション（毎月1日） | 1,980 | 3月は通信費、2・4月は新聞図書費 |
...
「同じ額で科目が違う」だけでは足りない。検証帳簿で試したところ、1,000円 の
...
検証帳簿（2026年）の地代家賃は、**大半が課税仕入10%（適格）で、1件だけ
非課税**だった。課税として扱っている額は 1,332,600円 で、控除額にすると
約 121,145円 になる。
...
「少数派が全体の N% 未満なら」のような絞りを入れたくなるが、**検証帳簿で誤検出が
...
取引先マスタが**すべて未確認**（`invoice_registration_no` も
`is_qualified_invoice_issuer` も空）だった。取り込み元の外部システム側でも
登録番号は全件未入力である。

取引先タグを付けた数件も、この意味では未確認のままだった。
**タグを付けた達成感で止まる形になっていた。**
...
検証帳簿は**入出金ベース**で記帳されていた。売上のすべてが「普通預金／
...
とき、`FindingKind::is_suspicion` を更新し忘れた。その結果、**検証帳簿の
...
どちらにせよ失敗するので気づけない。検証帳簿は不整合0件だったからこそ、
「疑いだけで失敗する」が表に出た。
...
検証帳簿の複製で12月31日に期首振替を入れたところ、**事業主貸 7,984,381円 と
事業主借 820,512円 が決算書から消え、`verify` は終了コード0のまま**だった。
...
——10% で 0.0909090909… という割り切れない小数になり、9,757,440円 の税額が
887,039円 になった（正しくは 887,040円）。

掛けてから割れば 975,744 ÷ 1.1 = 887,040 で割り切れる。

### 貸方を差し引く

返金・値引き（対価の返還）は課税仕入れ・課税売上を減らす。借方だけを足すと
控除額が過大になる。検証帳簿では返金の数件・49,380円 がこれに当たる。

### 集計できなかった明細を数えて言う

税区分の無い明細と、表に無いコードの明細を数える。**黙って捨てると、集計が
合っていないことに気づけない。** 検証帳簿では税区分の無い明細が128件あるが、
...
取引先に登録番号を後から入れられなかった。** 検証帳簿の14件はすべて空で、
