...
**トレードオフ**: `find_pending` は read model を200件引いてから ID で絞る。
「IDで1件引く」口が `ImportedTxQuery` に無いためで、明細が200件を超えると
古い側から漏れる。実運用（月あたり数十件）では足りるが、**引けなかった明細は
「見つかりません」になる**ので、必要になったら口を足す。
