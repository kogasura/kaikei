...
**検証**: 実帳簿の複製（検証用DBの TEMPLATE）に決算振替を記帳し、
...
2022-08-05 取得の「pc」97,200円 は、実際の帳簿で 2022〜2024年に 32,400円ずつ
...
実帳簿の通信費 386,880円 のうち、按分対象は携帯の 86,020円 だけである。
...
通信費の借方合計は 436,260円 だが、返金5件（49,380円）が貸方に立っており、
...
実帳簿の電動アシスト自転車等 105,600円（税込）は10万円以上だが、税抜なら
...
実帳簿では 248件 のうち5件がこれだった（ドメイン代の返金 49,380円）。
...
実帳簿の複製で12月31日に期首振替を入れたところ、**事業主貸 7,984,381円 と
...
控除額が過大になる。実帳簿では返金5件・49,380円 がこれに当たる。

### 集計できなかった明細を数えて言う

税区分の無い明細と、表に無いコードの明細を数える。**黙って捨てると、集計が
合っていないことに気づけない。** 実帳簿では税区分の無い明細が128件あるが、
...
取引先に登録番号を後から入れられなかった。** 実帳簿の14件はすべて空で、
