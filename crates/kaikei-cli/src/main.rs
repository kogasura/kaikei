...
async fn run_verify(fiscal_year: i32) -> Result<Vec<PathBuf>, String> {
    ...
    // **0 件なら、その年度に仕訳が無いことを言い切る。**
    //
    // 貸借対照表は前年から繰り越すので、仕訳が1件も無い年度でも残高は
    // 残る。実際 2027 年（まだ1件も記帳していない）を検査すると、減価償却・
    // 台帳・貸借の向きで4件の指摘が出た。**その年度の問題に見える。**
    //
    // いちばん危ないのは `--year` の打ち間違いである。もっともらしい出力が
    // 出るので、違う年度を見ていることに気づけない。`report` は同じ場合に
    // 断っているので、揃える。
    if output.entry_count == 0 {
        println!("  **この会計年度には仕訳が1件もありません。**");
        println!("    以下の指摘は、前年から繰り越した残高についてのものです。");
        println!("    年度の指定（--year）を確かめてください。");
    }
...
/// 0 で提出することになる。
///
/// **実際に試したら再現した。** 検証帳簿の複製で12月31日に期首振替を入れたところ、
...
fn invoices_to_collect(
    ...
    let mut rows: Vec<_> = entries
        ...
        .filter_map(|entry| {
            ...
            //
            // 検証帳簿では 248 件中5件がこれ（ドメイン代の返金 49,380円）。
...
///
/// 検証帳簿（2026-08-17）では、取引先マスタ14件すべてが未確認だった
...
///
/// 検証帳簿で言えば、外注工賃 320,000円 の相手は個人で、適格かどうかを
...
/// # なぜ件数だけでは足りないのか
///
/// 検証帳簿では 248 件と出る。**そのうち 176 件が旅費交通費**で、合計
...
/// 重複の疑いの内訳を出す。
///
/// **件数だけでは、所得に効くものと効かないものが混ざる。** 検証帳簿の
/// 26件は、余分な額の大半（840,000円）が事業主貸——引出しなので全部が
/// 誤りでも所得は1円も動かない。所得に効くのは 10,440円 しかなかった。
/// この差は件数を見ても分からない。
...
/// 期首残高のぶんだけ食い違う。**実際にそれで検証帳簿の健全な6科目を
/// 「不一致」と報告しかけた。**
///
...
/// 使ったところ、**税込経理の帳簿を税抜として扱ってしまった**。検証帳簿は
...
/// 二度と登録番号を入れられない**（検証帳簿の14件がその状態だった）。
///
/// # 名前は変えない
///
...
/// ことを誰も言わなくなった**（検証帳簿の複製で確かめた。1円入れただけで
/// 指摘が消えた）。
///
...
async fn write_reports(
    ...
    if !to_collect.is_empty() {
        ...
        // 件数だけを見せると全部が同じ手間に見えるが、検証帳簿では16件中
        // 3分の1ほどがこれで、残りは摘要から相手先を辿れた。
...
",
    ...
    // **本命。** 旅費交通費があれば案内を出す。
    //
    // 検証帳簿では「取引先が無い 248 件」のうち 176 件が旅費交通費で、
...
",
    ...
    //
    // 検証帳簿がこれ。2022年取得の pc（一括償却）は2024年で償却し終えている
    ...
    // **本命。** 科目の一部だけを按分できる。
    //
    // 検証帳簿の通信費 386,880円 のうち、按分対象は携帯の 86,020円 だけで、
    ...
    ///
    /// 検証帳簿がこれ（売上のすべてが「普通預金／売上高」で、売掛金の明細は0件）。
