...
impl FindingKind {
    ...
    /// を足したとき（D-120 / D-121）にここを更新せず、**検証帳簿の `verify` が
...
/// 「たまたま同額」を拾い始める（検証帳簿で試したところ、この幅を広げると
...
/// # 条件を絞る理由
///
/// 「同じ額で科目が違う」だけでは足りない。検証帳簿で試したところ、
...
/// # 少数派を「誤り」と呼ばない
///
/// 上の例では、多数派と少数派のどちらが誤りかが決まらない。**多数決で決めない。**
/// 件数を並べて、判断は人間に返す。
///
/// # 閾値を足していない
///
/// 「少数派が全体の N% 未満なら」のような絞りを入れたくなるが、**検証帳簿で
...
/// # なぜ件数だけでは足りないのか
///
/// 検証帳簿で 26 件と言われても手の付けようがない。**しかも件数は、所得に
/// 効くものと効かないものを混ぜている。** 実際の内訳はこうだった。
///
/// | 科目 | 余分な額 | 所得に効くか |
/// |---|---:|---|
/// | 事業主貸 | 840,000円 | **効かない**（引出し） |
/// | 旅費交通費 | 9,860円 | 効く |
/// | 支払手数料 | 580円 | 効く |
///
/// 目立つのは 84万円だが、**全部が誤りでも所得は1円も動かない**。逆に
/// 所得に効くのは 10,440円 しかない。この差は件数を見ても分からない。
#[derive(Debug, Clone, PartialEq, Eq)]
...
fn check_suspected_duplicates(entries: &[kaikei_core::JournalEntry]) -> Vec<Finding> {
    ...
    // 同じに見える。** 実際に外部の会計サービス側で見つかった二重計上は
...
mod tests {
    ...
    fn the_same_transaction_entered_twice_is_reported_as_suspected() {
        let entries = vec![
            entry_on(1, 1, 15, "ABC 5月分 請求", pair(550_000)),
            entry_on(2, 2, 15, "ABC 5月分 請求", pair(550_000)),
        ];
    ...
    // 300,000円 の重複が「ほか N 件」に隠れる。実際に外部の会計サービス側で見つかった
    ...
    ///
    /// 検証帳簿の動画サブスクリプションがこれ（2,280円・毎月2日・通信費と新聞図書費に
    ...
    ///
    /// 検証帳簿の地代家賃がこれ（大半が課税仕入10%で、1件だけ非課税）。
    /// **仕入税額控除の額が変わる。**
    #[test]
    ...
    /// 足したときに `is_suspicion` を更新せず、検証帳簿の `verify` が終了コード1で
    ...
    // **本命。** 資産の重複は所得に効かない。
    //
    // 検証帳簿では事業主貸の 840,000円 がこれだった。額はいちばん大きいが、
