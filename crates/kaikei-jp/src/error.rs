...
pub enum JpError {
    ...
    #[error(
        "決算処理に必要な科目（{role}）が勘定科目表に見つかりません: \"{code}\"。\
         勘定科目表にこの科目を追加するか、指定した科目コードを見直してください"
    )]
    ...
    #[error(
        "決算科目「{role}」に指定された科目コード \"{code}\" は見出し科目（postable: false）です。\
         見出し科目には記帳できないため決算振替仕訳を作れません。\
         記帳可能な科目コードを指定するか、勘定科目表の postable を見直してください"
    )]
    ...
    #[error(
        "決算科目「{role_a}」と「{role_b}」に同じ科目コード \"{code}\" が指定されています。\
         これらは別の科目である必要があります"
    )]
