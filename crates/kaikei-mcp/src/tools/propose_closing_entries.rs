...
応答には提案が2つ入ります。\
proposals は当年度末（12 月 31 日）に記帳する分で、収益と費用を元入金へ振り替えます。\
opening_proposals は翌年の 1 月 1 日に記帳する分で、\
事業主貸と事業主借をゼロにして差額を元入金へ振り替えます。\
opening_proposals を当年度末に記帳しないでください。\
青色申告決算書の貸借対照表には事業主貸と事業主借の欄があり、\
期末残高をそのまま書く様式なので、年内にゼロにすると様式と食い違います。\
