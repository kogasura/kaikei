...
`ROADMAP.md` の見積は「1週間」。実際には 2026-08-03 09:59Z（PR-A マージ）〜
2026-08-04（本 PR-I）の約1日で完了した。Phase 0〜2 と同じく AI エージェントに
...
| 完了条件 | 証跡 |
|---|---|
| **Claude Code から記帳できる** | 通し E2E が、科目の確認 → 記帳 → 読み戻し → 検索 → 元帳 → 試算表 → 逆仕訳 → 訂正済みの確認 → 記帳し直し、を `tools/call` だけで通す。README の手順（`.mcp.json` を含む）は PR-I で実際に実行して確かめた |
...
**2. 「実装が正しい」ことと「検査が実効的」ことは別だった（read model で8件）。**

PR-H の実装には誤りが無かったが、レビューが**書き換えを8つ名指しし、
そのいずれもが当時のテストを全て緑のまま通した**（すべて実測）。

| 書き換え | なぜ緑だったか |
|---|---|
| `ledger.rs` の期間合計を `entry_date >= from` → `> from` | 対照実装の `from` が「その日に明細が無い日」だった |
| `LEFT JOIN LATERAL` を素朴な `LEFT JOIN` に戻す | 土台に**二重訂正された仕訳が1件も無かった** |
| `reversed_by` の `ORDER BY` を降順に（最新の赤伝が付く） | 行数も残高も変わらないので、「2件返らない」しか見ていなかった |
| タグの絞り込みを AND → OR | 検査が**キー1つ**だったので AND と OR が一致した |
| 金額範囲から通貨の突き合わせを外す | 土台が**JPY 一色**だった |
| カーソルを `rows.last()` → `rows.first()` | `limit = 1` では先頭と末尾が同じ |
| `take(limit)` を落とす | 合計も並びも正しいままで、行数を見ていなかった |
| 続きの有無を `>` → `>=` | 余分な0件ページを1回踏んでも辿り終えた集合は変わらない |

原因は一貫している——**土台（seed）が実際の帳簿に似ていなかった。**
実帳簿には二重訂正も、同じ日に複数の明細も、複数のタグを持つ仕訳もあるが、
テストの土台には無かった。
