...
| `get_statements` | **要約**（区分の小計と合計。科目ごとの行を落とす） | 未実測 |
...
| `propose_closing_entries` | **要約**（件数・期間・`posted`。提案の明細を落とす） | 未実測 |

**「未実測」は文字どおり測っていないという意味です。** 上の数字は実際に
`SELECT octet_length(output::text)` で数えたもので、後から足した2件はまだ
同じ条件で測り直していません。**測っていない数字を書かない**ためにそう書いてあります。
