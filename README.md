...
**Claude Code のような MCP クライアントから実際に記帳でき、貸借対照表・損益計算書まで出せます。**
...
| Phase 4 | CSV 取込・証憑 | 未着手（`ROADMAP.md` の順を飛ばしています。記帳は MCP 経由で回せるため） |
| Phase 5 | 帳票・決算 | **着手中**。B/S・P/L と決算振替の提案が入り、ツールは13件になりました。帳簿の出力（CSV・印刷可能な形式）と青色申告決算書はまだです |
...
| `get_statements` | 貸借対照表と損益計算書。**帳簿の仕訳から組み立て直す**ので、`get_trial_balance`（SQL 集計）とは経路が違います。集計対象は指定期間の仕訳だけなので、**貸借対照表を出すときは会計年度の開始日を `from` に指定してください**（年度の途中からだと前期繰越が落ちます。落ちている疑いがあるときは `balance_sheet_note` が付きます） |
...
| `propose_closing_entries` | 決算振替仕訳（収益・費用のゼロ化と元入金への振替）を**提案**する。**記帳はしません**（応答の `posted` は常に `false`）。記帳するには返ってきた明細を `post_journal_entry` に渡します。集計期間は会計年度から導き、日付では指定できません。**期間の締めは行いません** |
...
- 非適格請求書の経過措置で**控除できない部分の帳簿処理**
  ...
  国税庁が示す原則的な取扱いは確認できていますが、消費税経理通達関係 Q&A が
  令和8年度税制改正（控除割合 70%）を反映していないため、**金額を確定させる
  実装は保留**しています（`docs/09-tax-research.md`）
- **簡易課税**を選択している場合に、仕入側の税額を消費税の申告集計から
  除外する処理と、売上への**事業区分**（第1〜6種）の記録。
  みなし仕入率そのものを記帳に反映しないのは仕様です（申告時の計算に
  用いるものであり、未実装という意味ではありません）
- **2割特例 / 3割特例**の課税期間ごとの選択
- **少額特例**（税込1万円未満）の税区分。適用可否が基準期間の課税売上高に
  依存し、それを保持する設定項目がまだありません
- **青色申告特別控除**（帳簿科目ではなく申告書上の控除。仕訳を作りません）
- 期首の振替仕訳、減価償却、棚卸、家事按分の年次調整

税理士に確認すべき事項は `docs/08-compliance.md` §9、一次情報で裏を取った
結果は `docs/09-tax-research.md` にあります。
...
| **読み取り系の `audit_log.output` は、8件中3件しか要約になっていません** | 下の「読み取りの記録が要約になっていない（5件）」を参照 |
...
| `get_statements` | **要約**（区分の小計と合計。科目ごとの行を落とす） | 432 バイト（空の帳簿。**区分の数だけで、科目数には比例しない**） |
...
参考（読み取り系ではないので上の「8件中3件」には数えていない）:

| ツール | `audit_log.output` の中身 | 1回あたり |
|---|---|---|
| `suggest_tax_category` | 応答本文がそのまま | 5,274 バイト |
| `propose_closing_entries` | **要約**（件数・期間・`posted`。提案の明細を落とす） | 300 バイト（提案の本数に依らない） |

後から足した2件（`get_statements` / `propose_closing_entries`）は**空の帳簿**で
測った値です。上の7件（仕訳1件の帳簿）とは条件が違いますが、どちらも
**要約なので帳簿の大きさに比例しません**——比例するのは要約していない5件のほうです。
